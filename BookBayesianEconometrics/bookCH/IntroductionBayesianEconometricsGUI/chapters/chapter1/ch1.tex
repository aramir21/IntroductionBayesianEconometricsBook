...
\begin{align}
	Var(\bm{\theta}|\mathbf{y})= \sum_{m=1}^{M}\pi(\mathcal{M}_m|\mathbf{y}) \widehat{Var} (\bm{\theta}|\mathbf{y},\mathcal{M}_m)+\sum_{m=1}^{M} \pi(M_m|\mathbf{y}) (\hat{\bm{\theta}}_m-\mathbb{E}[\bm{\theta}|\mathbf{y}])^2,
	\label{eq:128}
\end{align}

$\hat{\bm{\theta}}_m$ and $\widehat{Var}(\bm{\theta}|\mathbf{y},\mathcal{M}_m)$ are the posterior mean and variance under model $m$, respectively.
