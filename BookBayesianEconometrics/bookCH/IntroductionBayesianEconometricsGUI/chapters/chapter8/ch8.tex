...
Remember that we can run our GUI by typing
...
An important class of \textit{state-space models} is the \textit{Gaussian linear state-space model}, also known as, \textit{dynamic linear model}:
\begin{align*}
...
The weights are linked to the signal-to-noise ratio, that is, the proportion of the total variability ($\bm{\Omega}_t+\bm{\Sigma}_t$) due to the signal ($\bm{\Omega}_t$) versus the noise ($\bm{\Sigma}_t$). Note that in the simplest case where $M=K=1$, and $\bm{X}_t=\bm{G}_t=1$, then $\bm{K}_t=\bm{R}_t\bm{Q}_t^{-1}=(B_{t-1}+\Omega_t)/(B_{t-1}+\Omega_t+\Sigma_t)$. Thus, the weight associated with the observations is equal to 1 if $\Sigma_t=0$, that is, the posterior mean is equal to the actual observation. On the other hand, if $\Sigma_t$ increases compared to $\Omega_t$, there is more weight to the prior information, and consequently, the posterior mean is smoother as it heavily depends on the history. We ask in Exercise 1 to perform simulations with different signal-to-noise ratios to see the effects on the system.   
...
\begin{align*}
	...
	&=\bm{R}_{t}-\bm{R}_{t}\bm{X}_t^{\top}\bm{\Sigma}_t^{-1}\bm{X}_t\bm{R}_{t}\\
	&+ \bm{R}_{t}\bm{X}_t^{\top}\bm{\Sigma}_t^{-1}\bm{X}_t(\bm{R}_{t}^{-1}+\bm{X}_t^{\top}\bm{\Sigma}_t^{-1}\bm{X}_t)^{-1}\bm{X}_t^{\top}\bm{\Sigma}_t^{-1}\bm{X}_t\bm{R}_{t}\\
	&=\bm{R}_{t}-\bm{R}_{t}\bm{X}_t^{\top}\bm{\Sigma}_t^{-1}\bm{X}_t\bm{R}_{t}\\
	&+ \bm{R}_{t}\bm{X}_t^{\top}\bm{\Sigma}_t^{-1}\bm{X}_t[\bm{I}_K-(\bm{R}_{t}^{-1}+\bm{X}_t^{\top}\bm{\Sigma}_t^{-1}\bm{X}_t)^{-1}\bm{R}_{t}^{-1}]\bm{R}_{t}\\
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]  		 			
		...
		\State Set the hyperparameters of the \textit{precision of the state equations}: just one set of prior mean and variance parameters.
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
rm(list = ls()); set.seed(010101); T <- 200; sig2 <- 0.5^2
x <- rnorm(T, mean = 1, sd = 1); X <- cbind(1, x); B0 <- c(1, 0.5)
K <- length(B0); e <- rnorm(T, mean = 0, sd = sig2^0.5)
...
polygon(xx, yy, col="grey85", border="grey85")
xxML <- c(1:T, T:1)
yyML <- c(LimInfB2, rev(LimSupB2))
polygon(xxML, yyML, col="grey65", border="grey65")
lines(1:T, Bt[,2], col="black", lwd=2, lty=1)
lines(1:T, SmoothB2, col="black", lwd=2, lty=2)
lines(1:(T+1), colMeans(B2t), col="black", lwd=2, lty=3)
title("State vector: Slope parameter")
legend("bottomright", legend=c("True slope", "Smoothed slope (MLE)", "Posterior mean (Gibbs)", "95% band (Gibbs)", "95% band (MLE)"), lty=c(1,2,3,NA,NA), lwd=c(2,2,2,NA,NA), pch=c(NA,NA,NA,15,15), pt.cex=1, col=c("black","black","black","grey85","grey65"), bty="n", cex=0.7)
\end{lstlisting}
	\end{VF}
\end{tcolorbox} 

Figure \ref{fig1} shows the comparison between maximum likelihood (ML) and Bayesian inference. The light gray (Bayesian) and dark gray (maximum likelihood) shadows show the credible and confidence intervals at 95\% for the state slope parameter ($\beta_{t2}$). We see that the Bayesian interval encompasses the ML interval. This is a reflection of the extra uncertainty of the unknown variances. The black line is the actual trajectory of $\beta_{t2}$, the dashed and dotted lines are the \textit{smoothing} recursions using the ML and Bayesian estimates (posterior mean), respectively.\\ 
...
fan.col<-colorRampPalette(c("grey90","grey20"))
plot(NULL,main="Percentiles",xlim=c(1,T+1),ylim=c(-1,2),xlab="Time",ylab=TeX("$\\beta_{t1}$"))
fan(data=B2t,data.type="simulations",fan.col=fan.col,ln=NULL)
lines(colMeans(B2t),col="black",lwd=2,lty=1)
abline(h=0,col="grey30",lty=2,lwd=2)
legend("bottomright", legend=c("Posteriormean","Zeroline","Fanpercentiles"), lty=c(1,2,NA),lwd=c(2,2,NA), col=c("black","grey30",NA), pt.cex=2, pch=c(NA,NA,15),pt.bg=c(NA,NA,"grey60"), bty="n",cex=0.8)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
Let's start with the linear Gaussian model with autorregressive errors,
\begin{align}
...
Thus, we see that stochastic error $\mu_t$ follows an \textit{autoregressive process of order $p$}, that is, $\mu_t\sim AR(p)$. It is standard practice to assume that $\mu_t$ is second-order stationary, this implies that the mean, variance and autocovariance of $\mu_t$ are finite and independent of $t$ and $s$, although $\mathbb{E}[\mu_t\mu_s]$ may depend on $|t-s|$. Then, all roots of $\phi(L)$ lie outside the unit circle, for instance, given an $AR(1)$, then, $1-\phi_1L=0$, implies, $L=1/\phi_1$ such that  $|\phi_1|<1$ for the process being second-order stationary.
...
T <- length(yt); Xt <- cbind(diff(inf), diff(def))
Xtlag <- dplyr::lag(Xt, n = 1)
K <- dim(Xt)[2] + 1
...
# Basic density
library(ggplot2)
p <- ggplot(dfBinf, aes(x=var1)) + geom_density(color="grey", fill="grey") + geom_vline(aes(xintercept=mean(var1)), color="black", linetype="dashed", linewidth=1) + geom_vline(aes(xintercept=quantile(var1, 0.025)), color="black", linetype="dotted", linewidth=1) + geom_vline(aes(xintercept=quantile(var1, 0.975)), color="black", linetype="dotted", linewidth=1) + labs(title="Density effect of inflation on interest rate", x="Effect of inflation", y = "Density")
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
Setting $y_t=\mu_t$ such that $y_t=\sum_{s=1}^{p}\phi_jy_{t-s}+\sum_{s=1}^{q}\theta_s \epsilon_{t-s}+\epsilon_t$, letting $r=\max \left\{p,q+1\right\}$, $\phi_s=0$ for $s>p$ and $\theta_s=0$ for $s>q$, and defining $\bm{x}^{\top}=[1 \ 0 \ \dots \ 0]$, and $\bm{H}=[1 \ \theta_1 \ \dots \ \theta_{r-1}]^{\top}$ $r$-dimensional vectors, and 
\begin{align*}
	\bm{G}=\begin{bmatrix}
		\phi_1 & 1 & 0 & \dots & 0\\
		\phi_2 & 0 & 1 & \dots & 0\\
...
In addition, we can prove that setting $z_t=Y_t-\bar{\mu}$, we have $z_t=\phi_1z_{t-1}+\phi_2z_{t-2}+\epsilon_t$ where $\mathbb{E}[z_t]=0$, and these are equivalent representations (see Exercise 5). Then, setting  $\bm{x}^{\top}=[1 \ 0]$, $\bm{H}=[1 \ 0]^{\top}$, $\bm{G}=\begin{bmatrix}
	\phi_1 & 1\\
	\phi_2 & 0 \\
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]  		 			
		...
		\State Set the frequency: annual (1), quarterly (4), monthly (12)
...
The posterior distribution is given by\footnote{Take into account that we do not consider the first two observations when present the likelihood, this is not an issue when there is a large sample size.}
\begin{align*}
	\pi(\mu,\phi_1,\phi_2,\tau\mid \bm{y})&\propto \prod_{t=3}^T\exp\left\{-\frac{1}{2\exp(\tau)}(y_t-\mu-\phi_1y_{t-1}-\phi_2y_{t-2})^2\right\}\\
	&\times(\exp(\tau))^{-1/2}\exp\left\{-\frac{1}{2\sigma^2_{\mu}}(\mu-\mu_0)^2\right\}\\
	&\times\exp\left\{-\frac{1}{2\sigma^2_{\phi_1}}(\phi_1-\phi_{10})^2\right\}\\
	&\times\exp\left\{-\frac{1}{2\sigma^2_{\phi_2}}(\phi_2-\phi_{20})^2\right\}\times\exp\left\{-(\alpha_0/2+1)\tau\right\}\\
	&\times \exp\left\{-\delta_0/(2\exp(\tau))\right\}\exp(\tau).
\end{align*} 
...
\begin{align*}
	...
	\frac{\partial\log(\pi(\mu,\phi_1,\phi_2,\tau\mid \bm{y}))}{\partial\phi_1}&=\frac{\sum_{t=3}^T(y_t-\mu-\phi_1y_{t-1}-\phi_2y_{t-2})y_{t-1}}{\exp(\tau)}\\
	&-\frac{1}{\sigma_{\phi_1}^2}(\phi_1-\phi_{10})\\
	\frac{\partial\log(\pi(\mu,\phi_1,\phi_2,\tau\mid \bm{y}))}{\partial\phi_2}&=\frac{\sum_{t=3}^T(y_t-\mu-\phi_1y_{t-1}-\phi_2y_{t-2})y_{t-2}}{\exp(\tau)}\\
	&-\frac{1}{\sigma_{\phi_2}^2}(\phi_2-\phi_{20})\\	\frac{\partial\log(\pi(\mu,\phi_1,\phi_2,\tau\mid \bm{y}))}{\partial\tau}&=-\frac{(T-2)}{2}+\frac{\sum_{t=3}^T(y_t-\mu-\phi_1y_{t-1}-\phi_2y_{t-2})^2}{2\exp(\tau)}\\
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
# Simulation AR(2)
rm(list = ls()); set.seed(10101); T <- 1000; K <- 4 
...
The summary statistics of the posterior draws indicate that all 95\% credible intervals contain the true population parameters, and the posterior chains show clear evidence of convergence. Figure~\ref{fig4} displays the posterior results for the volatility process $h_t$. The posterior mean (dashed line) closely tracks the observed series (solid line), and the 95\% credible intervals (gray band) generally encompass the observed values.
...
require(latex2exp)
xx<-c(1:T,T:1)
yy<-c(df$lower,rev(df$upper))
plot(xx,yy,type="n",xlab="Time",ylab=TeX("$h_t$"))
polygon(xx,yy,col="grey85",border="grey85")
lines(1:T,df$x_true,col="black",lwd=1,lty=1)
lines(1:T,df$mean,col="black",lwd=1,lty=2)
title("Latentlog-volatility")
legend("bottomright", legend=c("Truelog-volatility","Posteriormean","95%credibleband"), lty=c(1,2,NA), lwd=c(2,2,NA), pch=c(NA,NA,15), pt.cex=1, col=c("black","black","grey85"), bty="n",cex=0.7)
\end{lstlisting}
	\end{VF}
\end{tcolorbox} 
...
plot_filtering_estimates<-function(df){
	p<-ggplot(df,aes(x=t))+
	geom_ribbon(aes(ymin=lower,ymax=upper,fill="Credibleband"),alpha=1,color=NA)+
	geom_line(aes(y=x_true,linetype="Trueseries"),color="black",linewidth=0.8)+
	geom_line(aes(y=mean,linetype="Posteriormean1"),color="black",linewidth=0.8)+
	geom_line(aes(y=meanT,linetype="Posteriormean2"),color="black",linewidth=0.8)+
	scale_linetype_manual(values=c("Trueseries"="solid","Posteriormean1"="dashed","Posteriormean2"="dotted"),name="Lines")+
	scale_fill_manual(values=c("Credibleband"="grey80"),name="")+
	ylab(TeX("$h_{t}$"))+xlab("Time")+theme_bw()+theme(legend.position="top")
	print(p)
}
...
The following code illustrates the procedure. We use the first 301 observations to estimate the model and reserve the last four observations to evaluate forecasting performance. Figures~\ref{fig6} and~\ref{fig7} display the impulse response functions of \textit{gs} with respect to \textit{gs}: the \textit{forecast-error impulse response} obtained under vague independent priors, and the \textit{orthogonalized impulse response} obtained using the Minnesota prior, respectively.

The forecasting results suggest that, in this example, both prior assumptions produce very similar effects. In particular, Figure~\ref{fig8} shows that the mean forecasts under the vague prior (dashed line) and the Minnesota prior (dotted line) are virtually indistinguishable from the true observations (solid line). However, in this specific case we observe that the predictive interval under the noninformative prior is more precise than that under the Minnesota prior: the 95\% predictive interval of the former (light gray shaded area) is narrower and fully contained within the 95\% interval produced by the latter (dark gray shaded area).
...
plot_IR<-function(df){
	p<-ggplot(df,aes(x=t))+
	geom_ribbon(aes(ymin=lower,ymax=upper,fill="Band"),alpha=1,color=NA)+
	geom_line(aes(y=mean,linetype="Mean"),color="black",linewidth=0.8)+
	scale_linetype_manual(values=c("Mean"="dashed"),name="Lines")+
	scale_fill_manual(values=c("Band"="grey80"),name="")+
	ylab("ImpulseResponse")+xlab("Time")+xlim(0,H)+
...
irMin <- bvartools::irf.bvar(objectMin, impulse = "gs", response = "gs", n.ahead = H, type = "oir", cumulative = FALSE) # Calculate IR
...
plot_FORE<-function(df){
	p<-ggplot(df,aes(x=t))+
	geom_ribbon(aes(ymin=lower1,ymax=upper1,fill="Band2"),alpha=1,color=NA)+
	geom_ribbon(aes(ymin=lower,ymax=upper,fill="Band1"),alpha=1,color=NA)+
	geom_line(aes(y=true,linetype="True"),color="black",linewidth=0.8)+
	geom_line(aes(y=mean,linetype="Baseline"),color="black",linewidth=0.8)+
	geom_line(aes(y=mean1,linetype="Minnesota"),color="black",linewidth=0.8)+
	scale_linetype_manual(values=c("True"="solid","Baseline"="dashed","Minnesota"="dotted"),name="Lines")+
	scale_fill_manual(values=c("Band1"="grey80","Band2"="grey60"),name="Bands")+
...
Algorithm \ref{alg:VAR} shows how to perform inference in VAR models using our GUI. See also Chapter \ref{chapGUI} for details regarding the dataset structure. 
