...
\begin{align*}
	\bm{Y}_t=\bm{a} + \sum_{l=1}^p\bm{A}_{l}\bm{Y}_{t-l}+\bm{\mu}_t,
\end{align*}
where $\bm{Y}_t$ is a $M$-dimensional vector having information of $M$ time series variables, $\bm{a}$ is a $M$-dimensional vector of intercepts, $\bm{A}_{l}$ are $M\times M$ matrices of coefficients, and $\bm{\mu}_t \stackrel{iid}{\sim} N_M(\bm{0}, \bm{\Sigma})$ stochastic errors, $t=1,2,\dots,T$ and $l=1,2,\dots,p$.

Following the matrix-form notation of the multivariate regression model (see sections \ref{sec44} and \ref{sec71}), we can set $\bm{Y}=\left[{\bm{Y_{1}}},{\bm{Y_{2}}},\ldots,{\bm{Y_{M}}}\right]$, which is an $ T\times M$ matrix, $\bm{x}_t=[1 \ \bm{Y}_{t-l}^{\top} \ \dots \ \bm{Y}_{tpP}^{\top}]$ is a $(1+Mp)$-dimensional row vector, we set $K=1+Mp$ to facilitate notation, and set \begin{align*}
\bm{X}=\begin{bmatrix}
	\bm{x}_1\\
	\bm{x}_2\\
	\vdots \\
...
\end{align*}
${\bm{S}}= ({\bm{Y}}-{\bm{X}}\widehat{\bm{B}})^{\top}({\bm{Y}}-{\bm{X}}\widehat{\bm{B}})$, and $\widehat{\bm{B}}= ({\bm{X}}^{\top}{\bm{X}})^{-1}{\bm{X}}^{\top}{\bm{Y}}$.

We also know from Section \ref{sec44} that the marginal distribution of $\bm{B}$ is $T_{K\times M}({\bm{B}}_n,{\bm{V}}_n,{\bm{\Psi}}_n)$ with $\alpha_n+1-M$ degrees of freedom.

Thus, we see that once we write a VAR(p) model in the right way, we can perform Bayesian inference as we did in the multivariate regression model. However, assuming conjugate prior has some limitations. For instance, VAR(p) models have many parameters, this implies loss of precision due to macroeconomic data no having large sample sizes, thus, it is desirable to impose prior restrictions in the specification of the model. This cannot be done using conjugate priors. In addition, natural conjugate priors do not allow for flexible extensions such as having different regressors in different equations. Thus, it is better to propose independent priors, that is, $\pi({\bm{B}},{\bm{\Sigma}})=\pi({\bm{B}})\pi({\bm{\Sigma}})$ where $\pi({\bm{B}})\sim N_{K\times M}({\bm{B}}_{0},{\bm{V}}_{0},{\bm{\Sigma}})$ and $\pi({\bm{\Sigma}})\sim IW({\bm{\Psi}}_{0},\alpha_{0})$.     
