...
We can extend the \textit{dynamic linear model} to take into account time invariant location parameters. In particular, we follow \cite{de1995simulation}, who propose the \textit{simulation smoother}. This algorithm overcomes some shortcomings of the FFBS algorithm, such as slow convergence and computational overhead. We focus on the case $M=1$, 
\begin{align}
	{Y}_t&=\bm{z}_t^{\top}\bm{\alpha}+\bm{x}_t^{\top}\bm{\beta}_t+\bm{T}_t\bm{\epsilon}_t,& t=1,2,\dots,T. & \text{   (Observation equation)}\nonumber\\
	\bm{\beta}_t&=\bm{G}_t\bm{\beta}_{t-1}+\bm{H}_t\bm{\epsilon}_t, & t=0,1,\dots,T. & \text{   (States equations)}\label{DeJongSt},
\end{align}
where $\bm{\beta}_0=\bm{0}$ and $\bm{\epsilon}_t\sim N(\bm{0}_{1+K},\sigma^2\bm{I}_{1+K})$.

Observe that this specification encompasses Equations \ref{eq1Obs} and \ref{eq1St} setting $\bm{\epsilon}_t=[\mu_t \ \bm{\omega}_t^{\top}]^{\top}$, $\bm{T}_t=[1 \ 0 \ \dots \ 0]$, $\bm{H}_t=[\bm{0}_K \ \bm{U}_{K\times K}]$ such that $\text{diag}\left\{\omega_1^2 \ \dots \ \omega_K^2\right\}=\sigma^2\bm{U}\bm{U}^{\top}$, $\bm{\alpha}=\bm{0}$, $\bm{G}_t=\bm{I}_K$, and $\bm{T}_t\bm{H}_t^{\top}=\bm{0}_{K}$.

The nice idea of \cite{de1995simulation} was to propose an efficient algorithm to get draws from $\bm{\eta}_t=\bm{F}_t\bm{\epsilon}_t$, where the most common choice is $\bm{F}_t=\bm{H}_t$, which means drawing samples from the state perturbation, and then, recovering the states from Equation \ref{DeJongSt} and $\bm{\beta}_0=\bm{0}$.  
