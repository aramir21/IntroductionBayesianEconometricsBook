...
In this example we use the population parameters to perform the filtering recursion. However, this is no the case in practice, that is, we have to estimate the time invariant parameters. Therefore, there are more elaborate algorithms to achieve this (see Chapter \ref{chap5}). A simple strategy is to estimate the time invariant parameters using MCMC algorithms up to a specific time period, and then update sequentially the state vector during other time period. In general, algorithms that combine sequential algorithms with MCMC algorithms seem to work well \cite{andrieu2010pmcmc}. See \cite{dahlin2019getting} for a tutorial of particle Metropolis-Hastings in \textbf{R}. 

Figure \ref{fig5} shows the filtering recursion using SMC with uneven weights (blue line), even weights (purple), bands given by plus/minus two standard deviations (lightblue shadow),\footnote{This standard deviation estimates the conditional posterior's standard deviation obtained via particles, not the estimator's standard deviation. The latter requires multiple independent particle runs on the same data.} and the true state (black line). It seems that SMC performs well using a simple version, and there are not significant differences between using even and uneven weights (see Chapter \ref{chap5}).
...
logalphas <- matrix(NA, N, T)   # Incremental imp. weights
cutoff <- 0.66
...
ggplot2::theme_set(theme_bw())
df <- tibble(t = seq(1, T), mean = FilterDist, lower = FilterDist - 2*SDFilterDist, upper = FilterDist + 2*SDFilterDist, meanT = FilterDistT, lowerT = FilterDistT - 2*SDFilterDistT, upperT = FilterDistT + 2*SDFilterDistT, x_true = h)
plot_filtering_estimates <- function(df) {
	...
	geom_line(aes(y = meanT), colour = "purple", linewidth = 0.5) +	ylab(TeX("$h_{t}$")) + xlab("Time")
	print(p)
}
plot_filtering_estimates(df)
\end{lstlisting}
