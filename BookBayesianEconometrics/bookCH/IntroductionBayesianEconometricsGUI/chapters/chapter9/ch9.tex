...
We can perform inference of the hierarchical longitudinal Poisson model in our GUI using Algorithm \ref{alg:HPosi}. Our GUI is based on the command \textit{MCMChpoisson} from the \textit{MCMCpack} package. We ask in Exercise 6 to use this package to perform inference in the number of visits to doctors (\textit{DocNum}) with the same specification of the example of \textbf{Doctor visits in Germany} of Section \ref{sec92}.
...
D <- diag(K2); bs1 <- rnorm(N, 0, sd = D[1]^0.5)
bs2 <- rnorm(N, 0, sd = D[2]^0.5); bs <- cbind(bs1, bs2)
...
Quantiles for each variable:
				2.5%     25%     50%     75%   97.5%
var1  0.08413  0.3828  0.5294  0.6679  0.9217
var2  0.24635  0.3216  0.3593  0.3985  0.4794
var3  0.41690  0.5007  0.5461  0.5884  0.6769
var4 -0.69640 -0.6162 -0.5706 -0.5272 -0.4464
summary(coda::mcmc(Ds))
Quantiles for each variable:
        2.5%     25%     50%      75%   97.5%
var1  0.3424  0.4777  0.5827  0.69957 1.01526
var2 -0.3323 -0.1924 -0.1287 -0.06925 0.04244
var3  0.1258  0.2216  0.2781  0.34702 0.53527
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
Use the package \textit{MCMCpack} to perform inference in the number of visits to doctors (\textit{DocNum}) with the same specification of the example of \textbf{Doctor visits in Germany} of Section \ref{sec92}.	
