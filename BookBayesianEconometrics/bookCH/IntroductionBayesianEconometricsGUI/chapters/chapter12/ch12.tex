...
\]
so the conditional probability of treatment jumps from $0$ to $1$ at $Z_i=c$. All units with $Z_i \ge c$ are treated, whereas those with $Z_i < c$ serve as the control group. Therefore, assignment is identical to treatment in the sharp RDD (perfect compliance). Figure \ref{Fig:SRDD} displays the conditional probability of treatment. 
...
\]
\[
Y_{in}(0)=\beta_{n0}+\mathbf{X}_i^{\top}\boldsymbol{\beta}_n+\mu_{ni}=\mathbf{w}_{in}^{\top}\beta_{n}+\mu_{ni},
\]
and
\[
Y_{ia}(1)=\beta_{a0}+\mathbf{X}_i^{\top}\boldsymbol{\beta}_a+\mu_{ai}=\mathbf{w}_{ia}^{\top}\beta_{a}+\mu_{ai},
\]
where $\mu_{jci}\sim t_v(0,\sigma^2_{jc})$ for $j\in\{0,1\}$, $\mu_{ni}\sim t_v(0,\sigma^2_{n})$, and $\mu_{ai}\sim t_v(0,\sigma^2_{a})$; here $t_v$ denotes the Student-$t$ distribution with $v$ degrees of freedom. The running variable is centered at the cutoff, so $Z_i=0$ at the threshold, and the FRD treatment effect is
\[
\tau_{\text{FRD}}=\beta_{1c1}-\beta_{0c1}.
\]

Under a Bayesian approach, posterior simulation simplifies because we sample each unit’s latent type ($C_i=\left\{c,n,a\right\}$). If we observe $Z_i<0$ and $D_i=0$, only two types are possible -compliers and never-takers- so
\[
P(C_i=c\mid Y_i, D_i=0,\boldsymbol{\theta})
=\frac{\eta_{c0}\,t_v\!\big(Y_i\mid \mathbf{w}_{ic}^{\top}\boldsymbol{\beta}_{0c},\sigma^2_{0c}\big)}
{\eta_{c0}\,t_v\!\big(Y_i\mid \mathbf{w}_{ic}^{\top}\boldsymbol{\beta}_{0c},\sigma^2_{0c}\big)+
	\eta_{n}\,t_v\!\big(Y_i\mid \mathbf{w}_{in}^{\top}\boldsymbol{\beta}_{n},\sigma^2_{n}\big)},
\]
where $\boldsymbol{\theta}$ is the set of parameters in the model, $\eta_{c0}$ and $\eta_{n}$ are the (current) mixture weights for a complier not taking treatment and for a never-taker, respectively.

Conversely, if we observe $Z_i\ge 0$ and $D_i=1$, only compliers and always-takers are possible, and
\[
P(C_i=c\mid Y_i, D_i=1,\boldsymbol{\theta})
=\frac{\eta_{c1}\,t_v\!\big(Y_i\mid \mathbf{w}_{ic}^{\top}\boldsymbol{\beta}_{1c},\sigma^2_{1c}\big)}
{\eta_{c1}\,t_v\!\big(Y_i\mid \mathbf{w}_{ic}^{\top}\boldsymbol{\beta}_{1c},\sigma^2_{1c}\big)+
	\eta_{a}\,t_v\!\big(Y_i\mid \mathbf{w}_{ia}^{\top}\boldsymbol{\beta}_{a},\sigma^2_{a}\big)},
\]
where $\eta_{c1}$ and $\eta_{a}$ are the mixture weights for a treated complier and an always-taker, respectively.

\cite{chib2016bayesian} assume a Dirichlet prior for the mixing probabilities



Other Bayesian approaches to inference in RD include \cite{branson2019nonparametric}, who use Gaussian process regression to model the conditional expectation function; \cite{rischard2020bayesian}, who develop a nonparametric Bayesian framework for spatial RDD; and \cite{chib2023nonparametric}, who develop the theory for nonparametric sharp and fuzzy RDD using basis-expansion methods.
...
\section{Other approaches}\label{sec12_3}
