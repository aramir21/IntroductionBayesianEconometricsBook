...
Under the assumption that the instrument $Z_i$ is independent of the potential outcomes (i.e., random assignment), and using the fact that the expectation of a binary variable equals its probability, the LATE can be expressed as:

\[
\tau_{LATE} =\frac{\mathbb{E}[Y_i \mid Z_i = 1] - \mathbb{E}[Y_i \mid Z_i = 0]}{\mathbb{E}[D_i \mid Z_i = 1] - \mathbb{E}[D_i \mid Z_i = 0]}.
\]

This is the standard instrumental variables (IV) estimand, often referred to as the \textit{Wald estimator}. It represents the ratio of the intention-to-treat effect on the outcome to the intention-to-treat effect on the treatment (i.e., the proportion of compliers). The instrument induces random variation in treatment: the numerator captures how outcomes respond to the instrument, and the denominator captures how treatment responds to the instrument. Dividing the two isolates the causal effect for compliers.
...
We can perform inference by imposing the weak exclusion restriction, setting $\eta_{n} = \eta_{n0} = \eta_{n1}$, so that the posterior distribution is
\[
...
Consider a situation with two time periods, $t = 1, 2$, and two groups: a treated group ($D_i = 1$) that receives the treatment between periods $t = 1$ and $t = 2$, and a control group that never receives the treatment ($D_i = 0$). We observe the outcomes $Y_{it}$ and treatment status $D_i$ for $i = 1, 2, \dots, N$. Assume a balanced panel data setting.

Let $Y_{it}(0,1)$ denote the potential outcome for unit $i$ in period $t$ if it is not treated in the first period but treated in the second period, whereas $Y_{it}(0,0)$ denotes the potential outcome when it is never treated. Thus, the observed outcome is
\[
Y_{it}=Y_{it}(0,1) D_i + Y_{it}(0,0) (1-D_i).
\]

The estimand of primary interest in difference-in-differences (DiD) is the average treatment effect on the treated (ATT),
\[
\tau_{ATT} = \mathbb{E}[Y_{it}(0,1) - Y_{it}(0,0) \mid D_i = 1].
\]

The key identification assumption in DiD is the \textit{parallel trends} assumption,
\[
\mathbb{E}[Y_{i2}(0,0) - Y_{i1}(0,0) \mid D_i = 1] = \mathbb{E}[Y_{i2}(0,0) - Y_{i1}(0,0) \mid D_i = 0],
\]
which states that, in the absence of treatment, the average change in outcomes for the treated and control populations would have evolved in the same way.

In addition, identification requires \textit{no anticipation effects},
\[
Y_{i1}(0,0)=Y_{i1}(0,1), \text{for all } D_i=1.
\]
This is, the treatment does not have causal effect prior to its implementation.
