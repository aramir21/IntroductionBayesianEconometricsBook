...
The \textit{parallel trends} assumption becomes
\begin{equation}
...
Following similar arguments as before, we obtain the \textit{conditional average treatment effect on the treated} (CATT):
\[
...
\[
\tau_2 = \mathbb{E}_{\mathbf{X}}[\tau_2(\mathbf{X}_i) \mid D_i = 1].
\]

Assuming a linear CEF and a homogeneous ATT, we can use either the first–difference specification
\[
\Delta Y_{i2} = \alpha + \tau_2 D_i + \sum_{k=1}^K \beta_k X_{ik} + \epsilon_i,
\]
where $\Delta Y_{i2}= Y_{i2} - Y_{i1}$, or the TWFE specification
\[
Y_{it} = \alpha + \alpha_i + \phi_t + \tau_2 \big[D_i \cdot \mathbbm{1}(t=2)\big]
+ \sum_{k=1}^K \beta_k X_{ik} + \epsilon_{it}.
\]

Note that these are \emph{not} equivalent. The first works in first differences and conditions on (typically time-invariant) $X_{ik}$, whereas the second is in levels with unit and time effects; with $\alpha_i$, time-invariant covariates are absorbed and cannot be separately identified. They coincide only in the special two-period case with no covariates (or when $X$ enters in differences), where regressing $\Delta Y_i$ on $D_i$ yields the same estimate of $\tau_2$ as the TWFE model.
...
However, if the treatment effect varies with pre-treatment covariates, the ATT estimator will be inconsistent \cite{abadie2005semiparametric}. Therefore, alternative \textit{semi-parametric} or \textit{non-parametric} approaches, such as \textit{inverse probability weighting} and \textit{doubly robust} estimators, are preferable, as they provide consistent estimators under conditional parallel trends \cite{roth2023whats}. For instance, \cite{breunig2025double} propose a double robust Bayesian inferential framework for average treatment effects.

Another limitation of DiD is that the parallel trends assumption is generally not robust to functional form transformations of the outcome. That is, it may hold when the outcome is measured in levels, but fail when it is expressed in logarithms. This is because two groups can have the same absolute increase (parallel trends in levels) but different percentage increases (non-parallel in logs).
...
\section{Staggered settings} 
treatment is an absorbing state

not-yet-treated or never-treated units
...
Parallel trend is not robust to functional forms, except in particular cases, like random assignment.
