...
Therefore, we can use pre-treatment variables to assess the balance between control and treated groups in terms of covariate levels ($X$) or trends ($\Delta X$) before and after treatment:
\[
...
\section{Staggered event studies}
...
The identifying assumptions for $ATT(t,g)$ are:

\textit{1. Staggered parallel trends}:
\begin{equation}
	\mathbb{E}[Y_{it}(\infty) - Y_{it'}(\infty) \mid G_i = g] 
	= \mathbb{E}[Y_{it}(\infty) - Y_{it'}(\infty) \mid G_i = g'], \quad \forall\, t, t' \geq g_{\min} - 1,
\end{equation}
where $g_{\min}$ is the earliest treatment period across all units.
...
Note that under these assumptions, we can express $ATT(t,g)$ in terms of observable outcomes as
\begin{align*}
	ATT(t,g) 
...
To study the dynamics of treatment effects, we can use \textit{event study} plots. These involve estimating and reporting effects for a range of periods before and after treatment, allowing us to analyze the temporal pattern of the ATTs \cite{baker2025did_guide}. In this context, we can perform inference on
\[
ATT_l^w = \sum_g w_g \, ATT(g, g+l),
\]
where $w_g$ denotes the weight for cohort $g$. For instance, if the outcome variable is defined at the school level (e.g., the proportion of dropouts), then $ATT(t,g)$ is a school-level effect, and $w_g$ could be the proportion of students in that school relative to the total student population.
