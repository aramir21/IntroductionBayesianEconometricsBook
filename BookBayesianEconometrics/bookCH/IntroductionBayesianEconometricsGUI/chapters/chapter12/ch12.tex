...
The hyperparameter $\beta_{0d}$ represents the hypothetical prior sample size. To specify a non-informative prior, we can set $\beta_{0d}=0$, making the posterior mean equal to the sample mean $\bar{y}_d$. Additionally, setting $\alpha_{0d} = \delta_{0d} = 0.01$ yields a weakly informative prior for the variances so that the posterior variances resemble the sampling variances. This non-informative prior setting implies that the predictive distribution is 
\[
Y(d) \mid \mathbf{y} \sim t\!\left(\bar{y}_d,\; \frac{(N_d+1)\delta_n}{N_d\alpha_n},\; \alpha_n\right).
\]
Consequently, ATE is 
\[
\text{ATE} = \mathbb{E}[Y(1) - Y(0)] = \bar{y}_1 - \bar{y}_0.
\]

Note that this result can be obtained under the simple linear regression framework by assuming non-informative prior distributions, so that the posterior mean of the location parameter coincides with the maximum likelihood estimator (see Exercise 1).

We can use simulation to obtain the posterior distribution of the average treatment effect (ATE). For example, assume that $\mu_1 = 15$, $\mu_0 = 10$, $\sigma_1^2 = 4$, $\sigma_0^2 = 2$, and $N_1=N_0=100$, and compute the posterior distribution of the ATE. The following code illustrates this procedure, while Figures \ref{fig12_0} and \ref{fig12_0a} display the predictive densities of the potential outcomes and the posterior distribution of the ATE. The posterior mean is 4.60, and the 95\% credible interval is (4.10, 5.12).
...
In this setting, the error term in the linear regression is defined as
\[
...
Thus, the LATE is the ratio between the intention-to-treat effect on the outcome (ITT), and the effect of the instrument on the treatment, which is the proportion of compliers. This ratio is known as the \textit{Wald estimator}. Thus, the instrument induces random variation in treatment. The numerator captures how outcomes change with the instrument, and the denominator captures how treatment changes with the instrument. Dividing isolates the causal effect for compliers.
...
\begin{align*}
	...
	&= \frac{12 + 9{,}663}{12 + 9{,}663 + 34 + 2{,}385} \\
	&= 0.8.
\end{align*}
This implies that $P(C_i = n) = 1 - P(C_i = c) = 0.2$.
...
Note that $\eta_{c0} = P(Y_i = 1 \mid C_i = c, Z_i = 0)$ and $\eta_{n0} = P(Y_i = 1 \mid C_i = n, Z_i = 0)$ cannot be point-identified. However, we can obtain \emph{set identification} because
\begin{align*}
	P(Y_i = 1 \mid Z_i = 0) 
	&= P(Y_i = 1 \mid Z_i = 0, C_i = c) \times P(C_i = c) \\
	&\quad + P(Y_i = 1 \mid Z_i = 0, C_i = n) \times P(C_i = n) \\
	&= \frac{P(Y_i = 1, Z_i = 0)}{P(Z_i = 0)} \\
	&= \frac{11{,}514}{11{,}514 + 74} \\
	&= 0.9936.
\end{align*}
...
This implies that 
\[
0.9936 = \omega \eta_{c0} + (1-\omega)\eta_{n0},
\]
which leads to 
\[
\eta_{n0} = 4.968 - 4\eta_{c0}.
\]
Given that probabilities must lie in the unit interval, i.e., $0 \leq 4.968 - 4\eta_{c0} \leq 1$, we obtain:  
(i) $\eta_{c0} \leq 1 \leq 1.242$ (automatically satisfied), and  
(ii) $4\eta_{c0} \geq 4.968 - 1 = 3.968$, which implies $\eta_{c0} \geq 0.992$.  

Consequently, $\eta_{c0} \in [0.992, 1]$ and $\eta_{n0} \in [0.968, 1]$.

These results imply that the \textit{complier average causal effect} (CACE),
\begin{align*}
	\tau_{CACE} &= P[Y_i = 1 \mid C_i = c, Z_i = 1] - P[Y_i = 1 \mid C_i = c, Z_i = 0] \\
	&= \eta_{c1} - \eta_{c0},
\end{align*}
is set-identified, with 
\[
\tau_{CACE} \in [-0.001, 0.007],
\]
...
Point identification can be achieved by imposing the \textit{weak exclusion restriction} \cite{imbens1997bayesian}, which states that treatment assignment is unrelated to potential outcomes for never-takers and always-takers. Under this assumption,
\[
P(Y_i = y \mid C_i = n, Z_i = z) = P(Y_i = y \mid C_i = n),
\]
so that $\eta_{n1} = \eta_{n0} = \eta_n = 0.986$. Substituting into the mixture equation yields $\eta_{c0} = 0.9955$, and consequently,
\[
\tau_{LATE} = 0.0035,
\]
...
We can perform inference imposing the weak exclusion restriction, which gives the local average treatment effect, the posterior mean is 0.003, and the 95\% credible interval is (0.0008,0.0054).
\begin{figure}[h!]
...
\cite{Conley2012} present practical methods for conducting sensitivity analysis of posterior results when relaxing the exclusion restriction in IV settings.
