...
of any other variable for which it is not a cause (\textit{causal Markov assumption}). See \cite{hernan2020causal} for a nice introduction, and the package \textit{dagitty} in \textbf{R} software to perform visualization and analysis of DAGs. Figure~\ref{DAG1} illustrates the causal structure underlying RCTs.
...
# Plot
hist(ppd1,col=grey(0.3,0.4),freq=FALSE,main="PosteriorPredictive",xlab="Y",xlim=c(5,25))
\end{lstlisting}
	\end{VF}
\end{tcolorbox} 
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
hist(ppd0,col=grey(0.7,0.4),freq=FALSE,add=TRUE)
legend("topright",legend=c("Treatment","Control"),fill=c(grey(0.3,0.4),grey(0.7,0.4)),border=NA)
...
hist(ate_draws, breaks = 50, freq = FALSE, main = "Posterior Distribution of ATE", xlab = "ATE (Y(1) - Y(0))", col = "gray80", border = "white")
abline(v = ate_mean, col = "black", lwd = 2)
abline(v = ci, col = "gray", lty = 2, lwd = 2)
legend("topright", legend = c("Posterior Mean", "95% Credible Interval"), col = c("black", "grey"), lwd = 2, lty = c(1, 2), bty = "n", cex = 0.8)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}  
...
We can use the framework in Section \ref{sec61} to estimate the average treatment effect of 401k eligibility on net financial assets. The following code shows how to get the posterior distribution of this effect. Figure \ref{fig12_1} displays the posterior distribution of the treatment effect, the 95\% credible interval is (USD 3,473, USD 8,371), and the posterior mean is USD 5,903.
...
geom_vline(xintercept = mean(beta_e401), color = "black", linetype = "dashed") +
labs(
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	fontupper=\large\bfseries,drop shadow southwest,sharp corners]\label{code2a_chap12}
	\textit{R code. Collider bias: Birth weight ``paradox"}
	\begin{VF}
		\begin{lstlisting}[language=R]
# Store results
...
gray_palette<-c("correct"="grey30","biased"="grey70")
ggplot(df_long,aes(x=estimate,fill=model,color=model)) + geom_density(alpha=0.5) + geom_vline(xintercept=true_effect, color="black", linetype="dashed", linewidth=1) + scale_fill_manual(values=gray_palette) + scale_color_manual(values=gray_palette) + labs(title="Posterior means across 100 simulations",x=expression(paste("Posterior mean of ",beta[S])), y="Density") + theme_minimal(base_size=14)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}  
...
We define $Y_i(z_i,D_i(z))$ to be the outcome for unit $i$ if exposed to treatment $D_i(z)$ after being assigned to treatment $z$. Therefore, the \textit{Intention-to-Treat} (ITT) causal effect of $Z_i$ on $Y_i$ is $\mathbb{E}[Y_i(1,D_i(1))-Y_i(0,D_i(0))]$, and can be expressed as:
\begin{align*}
	\underbrace{\mathbb{E}[Y_i(1,D_i(1))-Y_i(0,D_i(0))\mid D_i(1)=1,D_i(0)=1]\times P(D_i(1)=1,D_i(0)=1)}_{always-takers}\\
...
For compliers, $D_i = Z_i$, so we can simplify $Y_i(1,D_i(1)) = Y_i(1,1) = Y_i(1)$ and $Y_i(0,D_i(0)) = Y_i(0,0) = Y_i(0)$. Thus, $\mathbb{E}[Y_i(1) - Y_i(0)]$ is equal to
\begin{align*}
...
\eta_c = \Delta_D = \tfrac{11}{24}=0.46,
\]
thus,
\[
\eta_n = 1 - \eta_a - \eta_c = \tfrac{1}{6}=0.17.
\]

Then, we can \textit{pointidentified} the LATE under exclusion restriction,
...
hist(LATE, breaks = 40, freq = FALSE, main = "Posterior Distribution of CACE",
xlab = "CACE", col = "grey", border = "white")
abline(v = LATEmean, col = "black", lwd = 2)
abline(v = LATEci, col = "black", lty = 2, lwd = 2)
legend("topright", legend = c("Posterior Mean", "95% Credible Interval"), col = c("black", "black"), lwd = 2, lty = c(1, 2), bty = "n", cex = 0.8)
\end{lstlisting}
	\end{VF}
\end{tcolorbox} 
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	fontupper=\large\bfseries,drop shadow southwest,sharp corners]
	\textit{R code. Effects of vitamin A supplements on children's survival: Imposing exclusion restrictions}
	\begin{VF}
		\begin{lstlisting}[language=R]
...
# Plot posterior distribution of ATE
hist(LATEER, breaks = 40, freq = FALSE, main = "Posterior Distribution of LATE: Exclusion restrictions", xlab = "LATE", col = "grey", border = "white")
abline(v = LATEERmean, col = "black", lwd = 2) 
abline(v = LATEERci, col = "black", lty = 2, lwd = 2)
legend("topright", legend = c("Posterior Mean", "95% Credible Interval"), col = c("black", "black"), lwd = 2, lty = c(1, 2), bty = "n", cex = 0.8)
\end{lstlisting}
	\end{VF}
\end{tcolorbox} 
...
\begin{figure}[h!]
	...
	\caption[List of figure caption goes here]{Posterior distribution: Local average treatment effect vitamin A supplements.}\label{fig12_LATE}
\end{figure}
...
ggplot(df_beta, aes(x = effect)) + geom_density(fill = "grey", alpha = 0.6) +
geom_vline(xintercept = mean(beta_draws), color = "black", linetype = "dashed", linewidth = 1) + labs( title = "Posterior Distribution of 401(k) Participation Effect", x = expression(beta["p401"]), y = "Density") + theme_minimal(base_size = 14)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}  
...
\begin{figure}[h!]
	...
	\caption[List of figure caption goes here]{Posterior distribution: Local average treatment effect 401(k) participation on net financial assets.}\label{fig12_3}
\end{figure}
...
Assuming that parallel trends hold conditional on covariates, we obtain identification under the following assumptions \cite{baker2025did_guide}:
...
\begin{align*}
	ATT(g,t) &=\mathbb{E}[Y_{it} - Y_{ig-1} \mid G_i = g]-\mathbb{E}_{\mathbf{X}}\left\{\mathbb{E}[Y_{it} - Y_{ig-1} \mid\mathbf{X}_i,  G_i = g']\mid G_i=g\right\}.
\end{align*}
...
N <- 3000
Zi <- sample(seq(-24, 24, 1), N, replace = TRUE) # Running
Z <- Zi - mean(Zi)
...
ggplot(df, aes(x = LATE)) + geom_density(fill = "grey", alpha = 0.5, color = "grey") + geom_vline(xintercept = LATEmean, color = "black", linetype = "dashed", linewidth = 1) + geom_vline(xintercept = LATEci, color = "black", linetype = "dotted", linewidth = 1) + labs(title = "Posterior distribution of LATE", subtitle = paste0("Mean = ", round(LATEmean,3), " | 95% CI = [", round(LATEci[1],3), ", ", round(LATEci[2],3), "]"), x = "LATE", y = "Density") + theme_bw(base_size = 14)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}  
...
2. the parameters $\boldsymbol{\theta}$ and $\boldsymbol{\gamma}$ are \emph{a priori} independent,
\[
...
PostClat <- function(theta, sig12, omega11, i){
	if(i %in% idc1){
		...
		clat <- EnvStats::rnormTrunc(1, mean = mu, sd = sig2^0.5, min = 0, max = Inf)\end{lstlisting}
	\end{VF}
\end{tcolorbox}  
...
\begin{figure}[h!]
	...
	\caption[List of figure caption goes here]{Posterior distribution of the second parameter in the outcome equation: Selection vs no selection models}\label{fig12_SEL}
\end{figure}
...
\frac{d p_b^*(r)}{dr} = \frac{\alpha_2}{(\alpha_2 - \beta_2)(1+r)} > 0,
\]
\[
...
\]
to identify the demand and supply functions. Note that the demand equation is \textit{exactly identified}, whereas the supply equation is \textit{overidentified}.
...
Finally, observe that the doubly robust approach does not propagate the uncertainty arising from the estimation of the propensity score, which is treated as a plug-in. This omission is justified because the EIF is \textit{Neyman orthogonal}: estimation error in the propensity score enters only at second order and is therefore asymptotically negligible for uncertainty quantification. \cite{LiuYu2024QuasiBayesianCF} propose a quasi-Bayesian framework in which a \textit{control function}, explicitly modeling the source of endogeneity, is estimated in a first stage using Frequentist methods and then used in a second stage where inference is conducted using Bayesian methods. The posterior mean converges asymptotically to the true parameter, and a bootstrap procedure is employed to account for uncertainty propagation from the first stage, thereby achieving valid asymptotic coverage.\\
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	vals <- list(Bayes = qfun(Ate_nc_draws), `PA Bayes` = qfun(Ate_c_draws), `DR Bayes` = qfun(Ate_drb_draws))
