...
\]
that is, the average treatment effect on the treated (ATT) is the difference in outcome differences between treated and control units. See Figure 16.2 in \cite{chernozhukov2024applied} for a nice graphical representation of the identification assumptions in DiD.
...
Another common formulation in the linear regression setting that recovers the ATT is
\[
...
It is intuitive to propose the linear model:
\[
Y_{it} = \alpha + \alpha_i + \phi_t + \tau_2 \,\big[ D_i \cdot \mathbbm{1}(t = 2) \big] 
+ \big[ \mathbf{X}_i \cdot \mathbbm{1}(t = 2) \big]^{\top}\boldsymbol{\gamma} + \epsilon_{it}.
\]
However, recovering the ATT using this specification is generally not advisable, because the augmented model does not yield a consistent estimator under conditional parallel trends unless additional homogeneity assumptions are imposed \cite{abadie2005semiparametric}. For example, if the treatment effect varies with pre-treatment covariates, the ATT estimator will be inconsistent.
 
Therefore, alternative approaches such as \textit{regression adjustment}, \textit{inverse probability weighting}, and \textit{doubly robust} estimators are preferable, as they provide consistent estimators under conditional parallel trends \cite{roth2023whats}.
