...
For the outcome regression they place a Gaussian Process (GP) prior on $\eta^m$ (see Section \ref{13_4}), with a correction term that incorporates a preliminary estimator of the Riesz representer. The propensity component $\eta^\pi$ is estimated in a separate first step and is used in the correction/recentering; a prior for $\eta^\pi$ is therefore not required for their procedure.
...
Concretely, a GP prior is placed on a latent function $f(d,\mathbf{x})$ with a squared–exponential kernel, and the success probability is given by the logistic link $m(d,\mathbf{x})=\operatorname{logit}^{-1}\!\{f(d,\mathbf{x})\}$. Because the Bernoulli likelihood makes the required integrals over $f$ analytically intractable, the posterior is approximated using Laplace’s method (Expectation Propagation is an alternative). See Sections 3.3–3.5 of \cite{rasmussen2006gaussian} for details.
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]
		...
		\State Calculate the prior correction term $\sigma_n=\frac{\log(n)}{n^{1/2}\Gamma_n}$, $\Gamma_n=\frac{1}{n}\sum_{i=1}^n |\hat{\gamma}_{\eta}(d_i,\mathbf{x}_i)|$, $n$ is the effective sample size
		...
		\State Set an initial GP prior for  the adjusted mean $m_{\eta}^c(d,\mathbf{x})=\frac{1}{1+\exp\left\{-\eta^m(d,\mathbf{x},\sigma_n\times \hat{\gamma})\right\}}$		 			
		\For{\texttt{$s=1,\dots,S$}} 
		...
		\State Calculate the corrected posterior draw for the ATE $\tilde{\tau}_{\eta}^s=\tau_{\eta}^s-\hat{b}_{\eta}^s$, where $\tau_{\eta}^s=M^s_{ni}(m_{\eta}^{c,s}(1,\mathbf{x})-m_{\eta}^{c,s}(0,\mathbf{x}))$, and $\hat{b}_{\eta}^s=\frac{1}{n}\sum_{i=1}^n\tau[m_{\eta}^s(\mathbf{w}_i)-\hat{m}(\mathbf{w}_i)]$, $\tau[m]=m(1,\mathbf{x})-m(0,\mathbf{x})+\hat{\gamma}(d,\mathbf{x})(y-m(d,\mathbf{x}))$ 
		\EndFor 
	\end{algorithmic} 
\end{algorithm}      
