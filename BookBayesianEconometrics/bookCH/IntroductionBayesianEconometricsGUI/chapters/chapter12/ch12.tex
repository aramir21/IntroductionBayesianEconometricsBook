...
By contrast, a Bayesian inferential approach allows us to obtain \textit{distributional treatment effects}, which describe the entire distribution of potential outcomes under treatment and control, rather than just the mean. This has particular policy relevance; for example, it enables the estimation of the proportion of the population that benefits from a social program or the probability that a treated individual experiences a positive effect \cite{Ramirez2019a}.
...
Bayesian inference of causal effects in a completely randomized experiment with no covariates is then presented using the normal model in \cite{rubin1990neyman}. 

Let's assume that $p(Y_i(1),Y_i(0)\mid \boldsymbol{\theta})$ is normal with mean $\boldsymbol{\lambda}=[\lambda_1 \ \lambda_0]^{\top}$, variances $\sigma_1^2$ and $\sigma_0^2$, and correlation $\rho$. Note that the likelihood function does not have information about the correlation between the potential outcomes,
\[
p(\mathbf{y}\mid \mathbf{d}; \lambda_1,\lambda_0,\sigma^2_1,\sigma^2_0)=\prod_{i:d_i=1}\phi(\lambda_1,\sigma_1^2)\prod_{i:d_i=0}\phi(\lambda_0,\sigma_0^2),
\]
due to the  impossibility to observe the same unit under different treatment statuses simultaneously. Thus, most of the classical literature in treatment effects focused on average treatment effects due to these only requiring the marginal distributions of $Y(1)$ and $Y(1)$ \cite{heckman2014treatment}. However, a Bayesian inferential approach allows obtaining the distributional treatment effects, whose have a particular policy relevance due to they allow going beyond mean average treatment effects, for instance, the proportion of the population that benefits from a social program or the probability of a positive effect on the treated \cite{Ramirez2019a}.
