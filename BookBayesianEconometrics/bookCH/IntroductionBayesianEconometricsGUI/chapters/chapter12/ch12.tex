...
\cite{chernozhukov2003mcmc} propose the \textit{Laplace Type Estimator} (LTE), which can be seen as a particular case of the \textit{general Bayes posterior} \cite{bissiri2016general}, where the risk function is based on the moment conditions. 
...
\cite{chernozhukov2003mcmc} show that the posterior mean of the LTE is first-order equivalent to the efficient Generalized Method of Moments (GMM) estimator, and that the posterior quantiles yield asymptotically valid confidence intervals with correct frequentist coverage.\\
...
\[
q(D=1,\mathbf{X}=\mathbf{x},\tau)-q(D=0,\mathbf{X}=\mathbf{x},\tau),
\] 
where $q(D=d,\mathbf{X}=\mathbf{x},\tau)$ denotes the $\tau$-th quantile treatment response function.  

The potential outcomes are related to the quantile treatment response via
\[
Y_i(d)=q(D=d,\mathbf{X}=\mathbf{x},U(d)),
\]
where $U(d)\sim U(0,1)$ is the \textit{rank variable}. This variable captures unobserved heterogeneity that explains differences in outcomes given observed characteristics $\mathbf{x}$ and treatment $d$.  
...
Y_i(d)=q(D=d,\mathbf{X}=\mathbf{x},U(d)),
\]
where $q(D=d,\mathbf{X}=\mathbf{x},\tau)$ is strictly increasing in $\tau$ and $U(d)\sim U(0,1)$.  

\textit{2. Independence:} Conditional on $\mathbf{X}=\mathbf{x}$, the rank variables $\{U(d)\}$ are independent of the instruments $\mathbf{Z}$.  

\textit{3. Selection:} The treatment assignment is given by $D=\delta(\mathbf{Z},\mathbf{X},\mathbf{V})$ for some unknown function $\delta$ and unobserved heterogeneity $\mathbf{V}$.  

\textit{4. Rank invariance:} Conditional on $\mathbf{X}=\mathbf{x},\mathbf{Z}=\mathbf{z}$,

- either $\{U(d)\}$ coincide ($U(d)=U$ for all $d$), or
  
- $\{U(d)\}$ are identically distributed conditional on $\mathbf{V}$.  

\textit{5. Observables:} The observed data consist of $Y=q(D,\mathbf{X},U(D))$, $D$, $\mathbf{X}$, and $\mathbf{Z}$.  
...
\[
P\!\left(Y \leq q(D,\mathbf{X},\tau)\mid \mathbf{X},\mathbf{Z}\right)
= P\!\left(Y < q(D,\mathbf{X},\tau)\mid \mathbf{X},\mathbf{Z}\right)
= \tau,
\]
for all $\tau$ almost surely, and $U(D)\sim U(0,1)$ conditional on $(\mathbf{X},\mathbf{Z})$.  
...
Setting $\tau=0.5$ (median), the quantile treatment effect is 6,719, and the 95\% credible interval is (6,612, 6,806).
