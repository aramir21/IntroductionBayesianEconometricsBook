...
\subsection{Sharp regression discontinuity design (SRDD)}
...
Thus, the causal effect that we target in SRDD is
\[
...
\begin{align*}
	\lim_{z \to c^-} \Pr(D_i=1 \mid Z_i=z) \ne \lim_{z \to c^+} \Pr(D_i=1 \mid Z_i=z),\\
	0 \;<\; \lim_{z \to c^+} \Pr(D_i=1 \mid Z_i=z) \;-\; \lim_{z \to c^-} \Pr(D_i=1 \mid Z_i=z) \;<\; 1.
\end{align*}
Thus, the probability of treatment increases discontinuously at $c$. Note that in a fuzzy RDD, compliance is imperfect: some units assigned to treatment do not take it, and some units not assigned may nonetheless receive it. Figure \ref{Fig:FRDD} displays the conditional probability of treatment.
