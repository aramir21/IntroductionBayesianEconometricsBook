...
It is well known that, under model misspecification, that is, when the assumed likelihood function does not coincide with the true data-generating likelihood, the posterior concentrates its mass near those points in the support of the prior that minimize the Kullback--Leibler divergence with respect to the true model \cite{Kleijn2006}. However, this updated belief process may yield credible sets that fail to achieve the desired coverage, Bayes factors that can be misleading, and predictions that may remain approximately valid but require careful checking. In addition, the misspecified posterior distribution may exhibit suboptimal risk performance. In this setting, a modified posterior directly linked to the risk function of interest, known as the \textit{Gibbs posterior}, can still perform very well \cite{Jiang2008},
\begin{equation}\label{eq:GibbsPost}
p(\boldsymbol{\theta}|\mathbf{y})=\frac{\exp\left\{-NwR_N(\boldsymbol{\theta},\mathbf{y})\right\}\pi(\boldsymbol{\theta})}{\int_{\boldsymbol{\Theta}}\exp\left\{-NwR_N(\boldsymbol{\theta},\mathbf{y})\right\}\pi(\boldsymbol{\theta})d\boldsymbol{\theta}},
\end{equation}
where 
\[R_N(\boldsymbol{\theta},\mathbf{y})=\frac{1}{N}\sum_{i=1}^N l(\boldsymbol{\theta},\mathbf{y}_i)
\] 
is the average risk function, $w$ is know as the \textit{learning rate} or the inverse of the temperature in simulated annealing, and $N$ is the sample size.

In fact, Equation \ref{eq:GibbsPost} is a valid coherent mechanism to update prior beliefs \cite{bissiri2016general}. The point of departure is to set $\boldsymbol{\theta}$ that minimizes the expected loss function,
\[
L(\boldsymbol{\theta})=\int_{\mathcal{Y}}R_N(\boldsymbol{\theta},\mathbf{y})dF_0(\mathbf{y}),
\] 
where $F_0(\mathbf{y})$ is the unknown distribution function of the data $\mathbf{y}$. 


 

Introduce  as a generalization of \cite{chernozhukov2003mcmc}.
