...
In addition, it is important to note that the likelihood function does not contain information about the correlation between the potential outcomes due to \textit{the fundamental problem of causal inference}. Consequently, most of the classical literature on treatment effects has focused on average treatment effects, since these require only the marginal distributions of the potential outcomes $Y(1)$ and $Y(0)$ \cite{heckman2014treatment}.

By contrast, a Bayesian inferential approach allows us to obtain \textit{distributional treatment effects}, as formally defined by \cite{aakvik2005estimating}. These effects describe the entire distribution of potential outcomes under treatment and control in the population, rather than just the mean. This has particular policy relevance; for example, it enables the estimation of the proportion of the population that benefits from a social program or the probability that a treated individual experiences a positive effect \cite{Ramirez2019a}.
...
Thus, the LATE is the ratio between the intention-to-treat effect on the outcome (ITT), and the effect of the instrument on the treatment (\textit{Wald ratio}). Thus, the instrument induces random variation in treatment. The numerator captures how outcomes change with the instrument, and the denominator captures how treatment changes with the instrument. Dividing isolates the causal effect for compliers.
