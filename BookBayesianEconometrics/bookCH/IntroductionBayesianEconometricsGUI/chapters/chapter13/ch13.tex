...
# Compare true vs estimated
plot(beta_true, beta_post_mean, pch = 19, col = "steelblue", xlab = "True beta", ylab = "Posterior mean of beta", main = "SSVS Shrinkage")
abline(0, 1, col = "red", lty = 2)
		\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
# Estimate BART
BARTfit <- wbart(x.train = X.train, y.train = y.train, x.test = X.test, base = alpha, power = beta, k = k, sigdf = v, sigquant = q, ntree = J, ndpost = MCMCiter, nskip = burnin, keepevery = thinning)
# Trace plot sigma
keep <- seq(burnin + 1, MCMCiter + burnin, thinning)
df_sigma <- data.frame(iteration = 1:length(keep), sigma = BARTfit$sigma[keep])
ggplot(df_sigma, aes(x = iteration, y = sigma)) + geom_line(color = "black") + labs(title = "Trace Plot of Sigma", x = "Iteration", y = expression(sigma)) + theme_minimal()
# Prediction plot training
train_preds <- data.frame( y_true = y.train, mean = apply(BARTfit$yhat.train, 2, mean), lower = apply(BARTfit$yhat.train, 2, quantile, 0.025), upper = apply(BARTfit$yhat.train, 2, quantile, 0.975)) %>% arrange(y_true) %>% mutate(index = row_number())
ggplot(train_preds, aes(x=index)) + geom_ribbon(aes(ymin=lower, ymax=upper, fill="95% Interval"), alpha=0.4, show.legend=TRUE) + geom_line(aes(y=mean, color="Predicted Mean", linetype="Predicted Mean"), linewidth=0.8) + geom_line(aes(y=y_true, color="True y", linetype="True y"), linewidth=0.8) + scale_color_manual(name="Line", values=c("Predicted Mean"="black", "True y"="black")) + scale_linetype_manual(name="Line", values=c("Predicted Mean"="dashed", "True y"="solid")) + scale_fill_manual(name="Interval", values=c("95% Interval"="grey80")) + labs(title="Training Data: Ordered Predictions with 95% Intervals", x="Ordered Index", y="y") + theme_minimal()
...
ggplot(test_preds, aes(x=index)) + geom_ribbon(aes(ymin=lower, ymax=upper, fill="95% Interval"), alpha=0.4, show.legend=TRUE) + geom_line(aes(y=mean, color="Predicted Mean", linetype="Predicted Mean"), linewidth=0.8) + geom_line(aes(y=y_true, color="True y", linetype="True y"), linewidth=0.8) + scale_color_manual(name="Line", values=c("Predicted Mean"="black", "True y"="black")) + scale_linetype_manual(name="Line", values=c("Predicted Mean"="dashed", "True y"="solid")) + scale_fill_manual(name="Interval", values=c("95% Interval"="grey80")) + labs(title="Test Data: Ordered Predictions with 95% Intervals", x="Ordered Index", y="y") + theme_minimal() 
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
ggplot(importance_df, aes(x=variable, y=percent_used, linetype=trees)) + geom_line(color="black") + geom_point(color="black") + scale_linetype_manual(values=c("10"="solid", "20"="dashed", "50"="dotted", "100"="dotdash", "200"="longdash"), name="#trees") + scale_x_continuous(breaks=1:10) + labs(x="variable", y="percent used", linetype="#trees") + theme_minimal()
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
Perhaps the most commonly used covariance function in Gaussian Processes is the \textit{squared exponential} kernel (or \textit{radial basis function}) \cite{jacobi2024posterior}, defined as
\begin{align*}
	k(\mathbf{x}, \mathbf{x}') & = \sigma^2_f \exp\left(-\frac{1}{2l^2} \|\mathbf{x} - \mathbf{x}'\|^2\right),
\end{align*}
where \( \sigma^2_f \) is the signal variance, which controls the vertical variation (amplitude) of the function, \( l \) is the length-scale parameter, which determines how quickly the function varies with feature distance, and \( \|\mathbf{x} - \mathbf{x}'\|^2 \) is the squared Euclidean distance between the feature vectors \( \mathbf{x} \) and \( \mathbf{x}' \).
...
# Plot
ggplot(df, aes(x=x, y=value, linetype=draw)) + geom_line(linewidth=1, color="black") + scale_linetype_discrete(name="Function") + labs(title="Simulated Gaussian Process Draws", x="x", y="f(x)") + theme_minimal(base_size=14) + theme(legend.position="top")
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
f_i = \sin(2\pi x_i).
\]
The inference is based on the properties of the conditional Gaussian distribution (see below). Figure~\ref{GPsimPoints} shows that the posterior mean (solid black line) interpolates the observed points (black dots). Moreover, the level of uncertainty (light shaded area) increases in regions that are farther from the observed inputs, where the posterior mean tends to deviate more from the true underlying function (dashed line).
...
# Plot
ggplot(df, aes(x = x)) + geom_ribbon(aes(ymin = lower, ymax = upper), fill = "gray", alpha = 0.4) + geom_line(aes(y = mean), color = "black", linewidth = 1.2) + geom_line(aes(y = sample), color = "black", linewidth = 1, linetype = "dashed") + geom_point(data = obs, aes(x = x, y = y), color = "black", size = 3) + labs( title = "Gaussian Process with Conditioning Points", x = "x", y = "f(x)", caption = "Dashed: Posterior mean | Grey: 95% interval | Solid: Population | Black: Observed points" ) + theme_minimal(base_size = 14)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
In this section, we explore several methods developed to perform Bayesian inference when the sample size is large, particularly when there is a large number of observational units, commonly referred to as \textit{tall data setting}.
...
\begin{algorithm}[h!]
	\caption{Stochastic gradient Langevin dynamics algorithm}\label{Alg:SGLD}
	\begin{algorithmic}[1]
...
#### Parameters
K <- 10; N <- 100000
beta_true <- rep(0.5, K); B <- 5
batch_prop <- 0.01; Prior_prec <- 0.1
...
posterior_mh <- MCMClogit(formula, data = df, b0 = 0, B0 = Prior_prec, burnin = burnin, mcmc = n_iter)
...
# Plot
df_plot <- rbind(df_full, df_matrix, df_scalar, df_equal)
gray_palette <- c("overall" = "grey20", "matrix"  = "grey40", "scalar"  = "grey60",
"equal"   = "grey75")
ggpairs(df_plot, aes(color = method, fill = method, alpha = 0.4), upper = list(continuous = GGally::wrap("density", alpha = 0.4)), lower = list(continuous = GGally::wrap("density", alpha = 0.4)), diag  = list(continuous = GGally::wrap("densityDiag", alpha = 0.4))) + scale_color_manual(values = gray_palette) + scale_fill_manual(values = gray_palette) + theme_minimal()
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	fontupper=\large\bfseries,drop shadow southwest,sharp corners]
	\textit{R code. Stochastic Gradient Langevin Dynamics (SGLD): Logit regression}
	\begin{VF}
		\begin{lstlisting}[language=R]
...
df_plot <- data.frame( value = c(posterior_sgld[, k_target2], posterior_mh[, k_target2]), method = rep(c("SGLD", "MCMC"), each = n_iter))
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
gray_palette<-c("SGLD"="grey30","MCMC"="grey70")
ggplot(df_plot,aes(x=value,fill=method,color=method))+ geom_density(alpha=0.4)+ geom_vline(xintercept=beta_true[k_target2],linetype="dashed",color="black")+
scale_fill_manual(values=gray_palette)+ scale_color_manual(values=gray_palette)+ labs(title=expression(paste("Posterior ", "density ", "of ",beta[5])),x=expression(beta[5]),y="Density")+ theme_minimal()
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
\begin{figure}[!h]
	...
	\caption{Simulation: Posterior distributions from stochastic gradient Langevin Dynamics logit simulation exercise}
	\label{SGLDlogit}
\end{figure}  
