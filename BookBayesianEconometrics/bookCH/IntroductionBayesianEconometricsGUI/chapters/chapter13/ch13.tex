...
We predict $Y$ using $\hat{f}(\mathbf{w})$, a model trained from the data. Thus, the expected squared error (ESE) at a fixed set of features is given by
\begin{align*}
...
\end{align*}
where $\mathbb{E}_{\mathcal{D},\boldsymbol{\mu}} \left[\mu_i(f(\mathbf{w}_i) - \hat{f}(\mathbf{w}))\right] = 0$ by independence, and $\bar{f}(\mathbf{w}_i) = \mathbb{E}_{\mathcal{D}}[\hat{f}(\mathbf{w}_i)]$.
...
Given the popularity of the LASSO as a variable selection technique, we present its Bayesian formulation in this subsection \cite{Park2008}.
