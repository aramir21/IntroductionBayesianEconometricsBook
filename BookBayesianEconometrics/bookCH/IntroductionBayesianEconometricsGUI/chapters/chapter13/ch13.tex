...
In this section, we explore several methods developed to perform Bayesian inference when the sample size is large—specifically, when there is a large number of units, commonly referred to as \textit{tall datasets}.

Performing Bayesian inference in such settings is particularly challenging because each iteration of an MCMC algorithm requires computations over all \( N \) observations when evaluating the likelihood function. One alternative is to use \textit{Variational Bayes} (see Chapter~\ref{chap15}); however, this approach can sometimes be difficult to implement or may have limitations in terms of uncertainty quantification.

In scenarios where the observations are assumed to be independent, two major frameworks have been proposed to scale MCMC algorithms: \textit{divide-and-conquer approaches} and \textit{subsampling-based algorithms}. Divide-and-conquer methods partition the dataset into batches, perform MCMC separately on each subset, and then integrate the results to approximate the full posterior. In contrast, subsampling-based algorithms aim to reduce the number of data points used to evaluate the likelihood function at each MCMC iteration. For an excellent review of these frameworks, see \cite{bardenet2017markov}.
