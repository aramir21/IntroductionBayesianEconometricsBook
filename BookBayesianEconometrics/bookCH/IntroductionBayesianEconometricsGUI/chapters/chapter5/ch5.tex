...
To simplify concepts let's focus on a parameter space composed by two blocks, $\bm{\theta} = [\bm{\theta}_1 \ \bm{\theta}_2]^{\top}$, the Gibbs sampling algorithm uses as transition kernel $q(\bm{\theta}_1^{(s)},\bm{\theta}_2^{(s)}\mid \bm{\theta}_1^{(s-1)},\bm{\theta}_2^{(s-1)})=\pi(\bm{\theta}_1^{(s)}\mid \bm{\theta}_2^{(s-1)},\bm{y})\pi(\bm{\theta}_2^{(s)}\mid \bm{\theta}_1^{(s)},\bm{y})$. Thus,
...
summary(mcmc(kk))
hist(kk, main = "Histogram: Posterior mean change point", xlab = "Posterior mean", col = "black", breaks = 25)
\end{lstlisting}
	\end{VF}
\end{tcolorbox} 

The posterior results indicate that the rate of disasters decreases from 3.1 to 0.92 per year in 1890. 
...
h <- hist(p, breaks=50, col="gray", xlab="Proportion Ph.D. students sleeping at least 6 hours", main="Beta draws from a Metropolis-Hastings algorithm")
pfit <- seq(min(p),max(p),length=50)
yfit<-dbeta(pfit, an, bn)
yfit <- yfit*diff(h$mids[1:2])*length(p)
lines(pfit, yfit, col="black", lwd=2)
\end{lstlisting}
	\end{VF}
\end{tcolorbox} 
...
As a toy example, let's compare the Gibbs sampling, M-H and HMC algorithms when the posterior distribution is a bi-variate Gaussian distribution with mean $\bm{0}$ and covariance matrix $\bm{\Sigma}=\begin{bmatrix}
	1 & \rho \\
	\rho & 1
...
Figure \ref{fig54} shows the posterior draws of $\theta_1$ using the Gibbs sampler (Panel A, left), the Metropolis-Hastings algorithm (Panel B, middle), and the Hamiltonian Monte Carlo (Panel C, right). The convergence diagnostic plots (not shown) suggest that the three algorithms perform a good job. Although the acceptance rate in HMC is higher than the M-H due to the HMC producing larger changes in $\bm{\theta}$ than a corresponding number of random-walk M-H iterations \cite{neal2011mcmc}.
...
Note that we could alternatively arrive to these weights as follows 
...
h <- hist(thetaCauchyN, breaks=50, col="grey", xlab="x", main="Cauchy draws from importance sampling: Normal standard proposal")
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
h <- hist(thetaCauchyT, breaks=50, col="grey", xlab="x", main="Cauchy draws from importance sampling: Student's t proposal")
pfit <- seq(min(thetaCauchyT),max(thetaCauchyT),length=50)
yfit<-dcauchy(pfit)
yfit <- yfit*diff(h$mids[1:2])*length(thetaCauchyT)
lines(pfit, yfit, col="black", lwd=2)
plot(wNstars, main = "Importance sampling: Cauchy distribution", ylab = "Weights", xlab = "Iterations", pch = 16, col = "black")
points(wTstars, pch = 17, col = "black")
legend("topright", legend = c("Normal", "Student's t"), pch = c(16, 17), col    = "black")
		\end{lstlisting}
	\end{VF}
\end{tcolorbox} 
 

Figure \ref{fig55} shows the weights using the standard normal distribution (black dots) and the Student's t-distribution with 3 degrees of freedom (black triangles) as proposals. We observe that a few draws carry too much weight when using the normal proposal; this occurs because the normal distribution has much lighter tails compared to the Cauchy distribution. In contrast, using the Student's t-distribution with 3 degrees of freedom improves this situation.
...
plot_filtering_estimates <- function(df) {
	ggplot(data = df, aes(x = t)) + geom_ribbon(aes(ymin = lower, ymax = upper, fill = "Uncertainty band"), alpha = 1) + geom_line(aes(y = theta_true, linetype = "True state"), color = "black", linewidth = 0.6) + 	geom_line(aes(y = mean, linetype = "Filter mean"), color = "black", linewidth = 0.6) + scale_fill_manual(values = c("Uncertainty band" = "grey80")) + 	scale_linetype_manual(values = c("True state" = "solid", "Filter mean" = "dotted")) + labs(fill = "", linetype = "") + ylab(TeX("$\\theta_{t}$")) + xlab("Time") + theme_bw() + theme(legend.position = "top")
}
...
Figure \ref{fig58} shows the trajectory of the true state vector (black line), the posterior mean (dotted line), and the area defined by $\pm2\hat{\sigma}_{\theta}$ (gray shaded area).
...
Let's perform particle filtering in this example. The following code illustrates the procedure. Figure \ref{fig510} shows the performance of particle filtering in this example. There is the true state vector (black line), the means based on $\left\{\bm{\theta}_{1:t}^{(s)},w_t^{*(s)}\right\}$ (dotted line) and  $\left\{\bm{\theta}_{1:t}^{r(s)},1/S\right\}$ (dashed line), and the area defined by $\pm2\hat{\sigma}_{\theta}$ based on the former (light blue shaded area). Note that the particle filtering algorithm has better performance than the SIS algorithm.   
...
require(latex2exp)
ggplot2::theme_set(theme_bw())
df <- tibble(t = 1:T, mean = FilterDist, lower = FilterDist - 2*SDFilterDist, upper = FilterDist+ 2*SDFilterDist, meanT = FilterDistT, lowerT = FilterDistT - 2*SDFilterDistT, upperT = FilterDistT + 2*SDFilterDistT, x_true = theta_true)
plot_filtering_estimates<-function(df){
	ggplot(df,aes(x=t))+ geom_ribbon(aes(ymin=lower,ymax=upper,fill="Interval1"),alpha=1,color=NA)+
	geom_ribbon(aes(ymin=lowerT,ymax=upperT,fill="Interval2"),alpha=0.5,color=NA)+
	geom_line(aes(y=x_true,linetype="TrueState"),color="black",linewidth=0.7)+
	geom_line(aes(y=mean,linetype="Filter1"),color="black",linewidth=0.7)+
	geom_line(aes(y=meanT,linetype="Filter2"),color="black",linewidth=0.7)+
	scale_linetype_manual(values=c("TrueState"="solid","Filter1"="dotted","Filter2"="dashed"),name="Lines")+
	scale_fill_manual(values=c("Interval1"="grey80","Interval2"="grey90"),name="Bands")+
	ylab(TeX("$\\theta_{t}$"))+xlab("Time")+theme_bw()+theme(legend.position="top")
}
...
\begin{itemize}
	...
	\item Check if there are errors in the posterior simulator of the Metropolis-Hastings algorithm in this example using the Geweke approach, with the first moments of $p$ and $p2$ as test functions. Remember from Exercise 15 in Chapter \ref{chap4} that the sample size is 52, and $\alpha_0=1.22$ and $\beta_0=2.57$.
