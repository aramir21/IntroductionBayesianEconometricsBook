...
Hamiltonian Monte Carlo (HMC) was proposed by \cite{duane1987hybrid} and later introduced to the statistical community by \cite{neal1996bayesian}. HMC is an extension of the Metropolis algorithm designed to efficiently explore the parameter space by introducing \textit{momentum variables}. This feature helps overcome the random walk behavior of Gibbs sampling and the M-H algorithm. HMC, also known as hybrid Monte Carlo, is particularly advantageous for high-dimensional posterior distributions, as it reduces the risk of getting stuck in local modes and significantly improves mixing \cite{neal2011mcmc}. 

However, HMC is designed to work with strictly positive target densities. Therefore, transformations are required to handle bounded parameters, such as variances and proportions. For instance, logarithmic and logit transformations can be applied in these cases, respectively. This requires the use of the change-of-variable theorem to compute the log posterior density and its gradient, which are necessary to implement the HMC algorithm.

HMC leverages concepts from physics, specifically Hamiltonian mechanics, to propose transitions in the Markov chain. In Hamiltonian mechanics, two key variables define the total energy of the system along a particular trajectory: the \textit{position} ($\bm{\theta}$) and the \textit{momentum} ($\bm{\delta}$). The Hamiltonian represents the total energy of the system, which is the sum of \textit{potential energy} (energy stored in the system due to position) and \textit{kinetic energy} (energy associated with motion). The goal is to identify trajectories that preserve the total energy of the system, meaning the system's evolution keeps the Hamiltonian invariant, while avoiding trajectories that fail to do so. This approach enhances the acceptance rate of proposed transitions.

To implement HMC, we solve the differential equations derived from the Hamiltonian, which are based on the derivatives with respect to position and momentum. However, these equations rarely have analytical solutions, requiring the use of numerical methods. This necessitates discretization of Hamilton’s equations, which introduces errors. To mitigate these errors, HMC employs the \textit{leapfrog integrator}, a numerical method with lower errors compared to simpler approaches like the Euler method.

In particular, HMC uses a \textit{momentum variable} ($\delta_k$) that accompanies each $\theta_k$, such that the transition kernel of $\bm{\theta}$ is determined largely by the variables in $\bm{\delta}$. Both vectors are updated using a Metropolis algorithm at each stage such that HMC updates leaves the distribution invariant \cite{neal2011mcmc}.

HMC involves the joint density $p(\bm{\theta}, \bm{\delta} | \bm{y}) = \pi(\bm{\theta} | \bm{y}) \times p(\bm{\delta})$, where $\bm{\delta} \sim N(\bm{0}, \bm{M})$, and $\bm{M}$ is a diagonal matrix such that $\delta_k \sim N(0, M_{kk})$. In easy situations $\bm{M}=\bm{I}$, in more complex problems, $\bm{M}$ is scaled to approximate the inverse of the posterior covariance matrix evaluated at the maximum a posterior estimate.

Algorithm \ref{Alg:HMC} outlines the implementation of the HMC algorithm. Note that the gradient vector $\frac{d\log(\pi(\bm{\theta}|\bm{y}))}{d\bm{\theta}}$ must be computed analytically, as using finite differences can be computationally expensive. However, it is advisable to verify the analytical calculations by evaluating the gradient at the maximum posterior estimate, where the function should return values close to 0, or by comparing results with finite differences at a few evaluation points. Additionally, observe that we do not retain $\bm{\delta}$ since it is updated at the beginning of each iteration and is not required subsequently.
...
The number of Leapfrog steps ($L$) and the step size ($\epsilon$) are chosen such that $\epsilon \times L \approx 1$. For instance, $\epsilon$ is randomly drawn from a uniform distribution between 0 and $2\epsilon_0$, and $L$ is the largest integer near $1/\epsilon$. Start with $\epsilon_0 = 0.1$. While these parameters can be fine-tuned to achieve the optimal acceptance rate of approximately 65\% \cite[Chap.~12]{gelman2021bayesian}—for instance, increasing $\epsilon_0$ if the acceptance probability is much higher than 65\% or decreasing it if it is much lower—the final analysis should be based on posterior draws where these parameters remain fixed. Lastly, the number of iterations ($S$) is selected to ensure convergence to the stationary distribution.\\
