...
\end{align}
where $\bm{\theta}^{(s)}$ are draws from $\pi(\bm{\theta}|\bm{y})$. The \textit{strong law of large numbers} shows that $\bar{h}(\bm{\theta})S$ converges (almost surely) to $\mathbb{E}{\pi}[h(\bm{\theta})]$ as $S \rightarrow \infty$.
...
We know from the Bayes' rule that $$p(\bm{y})^{-1}=\frac{\pi(\bm{\theta}|\bm{y})}{p(\bm{y}|\bm{\theta})\times \pi(\bm{\theta})},$$
then, 
\begin{align*}
...
\end{align*}
Thus, an estimate of the marginal likelihood is $\left[\frac{1}{S}\sum_{s=1}^S\frac{q(\bm{\theta}^{*(s)})}{p(\bm{y}|\bm{\theta}^{*(s)})\times\pi(\bm{\theta}^{*(s)})}\right]^{-1}$. This is Gelfand-Dey method to calculate the marginal likelihood \cite{gelfand1994bayesian} (see subsection \ref{sec10_4_3}).\\

\textbf{Example: Cauchy distribution}

Let's sample from a Cauchy distribution, $C(0,1)$, using as proposal a standard normal distribution and a Student's t distribution with 5 degree of freedom.
