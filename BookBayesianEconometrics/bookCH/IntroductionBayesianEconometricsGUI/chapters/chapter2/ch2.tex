...
\section{Why is the Bayesian approach not that popular?}\label{sec25}
...
alpha <- N - 1
serror <- (s2/N)^0.5 
LimInf <- y_bar - abs(qt(0.025, alpha)) * serror # Lower bound
LimSup <- y_bar + abs(qt(0.025, alpha)) * serror # Upper bound
y.cut <- 103
P <- 1-metRology::pt.scaled(y.cut, df = alpha, mean = y_bar, sd = serror) # Probability of mu greater than y.cut
\end{lstlisting}
\end{VF}
\end{tcolorbox}
...
\begin{enumerate}
	\item \textbf{Jeffreys-Lindley paradox}
	
	The \textbf{Jeffreys-Lindley paradox} \cite{Jeffreys1961,lindley1957statistical} represents an apparent disagreement between the Bayesian and Frequentist frameworks in a hypothesis testing scenario.
