...
\begin{algorithm}[!h]
	\caption{Bayesian bootstrap in linear regression}
	\label{alg:BB}
	\begin{algorithmic}[1]
		...
		\State Sample $(y_i,\bm{x}_i^{\top})$ $S_1$ times with replacement and probabilities $g_i$, $i=1,2,\dots,N$.
		...
		\State Repeat this process $S_2$ times.
...
In this chapter, we present the core univariate regression models and demonstrate how to perform Bayesian inference using Markov Chain Monte Carlo methods. Specifically, we cover a mix of algorithms: Metropolis-Hastings, Gibbs sampling, nested M-H, and M-H-within-Gibbs. These algorithms form the foundation for performing Bayesian inference in more complex settings using cross-sectional data sets
...
\begin{enumerate}
	...
	Simulate an ordered probit model where the first regressor distributes $N(6, 5)$ and the second distributes $G(1,1)$, the location parameter is $\bm{\beta}=\left[0.5 \ -0.25 \ 0.5\right]$, and the cutoffs is the vector $\bm{\alpha}=\left[0 \ 1 \ 2.5\right]$. Program from scratch a Metropolis-within-Gibbs sampling algorithm for this simulation.
