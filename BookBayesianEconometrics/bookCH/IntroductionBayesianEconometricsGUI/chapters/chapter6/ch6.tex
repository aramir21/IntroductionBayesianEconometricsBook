...
\begin{algorithm}[!h]
	...
	\begin{algorithmic}[1]
		...
		\State Estimate $\bm{\beta}$ using ordinary least squares in the model $\mathbb{E}(\bm{y}\mid \bm{X})=\bm{X}\bm{\beta}$, $\bm{y}$ being an $S_1$ dimensional vector, and $\bm{X}$ an $S_1\times K$ matrix from the previous stage.$^*$ 
