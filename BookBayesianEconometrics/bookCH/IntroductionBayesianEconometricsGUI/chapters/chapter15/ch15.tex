...
where \( F = P(U \leq u) \), and \( Q (p\mid{\theta}) \) represents the \( p \)-quantile \cite{rayner2002numerical}. The quantile function of the g-and-k distribution is given by
...
######### ABC Exchange rate of returns: USD/EURO
...
dentheta <- ggplot(df1, aes(x=Value, linetype=Distribution)) + geom_density(linewidth=1, color="black") + scale_linetype_manual(values=c("AR"="solid","MCMC"="dashed","SMC"="dotted")) + labs(title=TeX("Posterior density plot: $\\theta$"), x=TeX("$\\theta$"), y="Posterior density") + theme_minimal() + theme(legend.title=element_blank())
df2 <- data.frame(Value = c(PostABCAR[1:Sp,4], PostABCMCMC[1:Sp,4], PostABCSMC[1:Sp,4]),
Distribution = factor(c(rep("AR", Sp), rep("MCMC", Sp), rep("SMC", Sp))))
deng <- ggplot(df2, aes(x=Value, linetype=Distribution)) + geom_density(linewidth=1, color="black") + scale_linetype_manual(values=c("AR"="solid","MCMC"="dashed","SMC"="dotted")) + labs(title="Posterior density plot: g", x="g", y="Posterior density") + theme_minimal() + theme(legend.title=element_blank())
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
denk <- ggplot(df3, aes(x=Value, linetype=Distribution)) + geom_density(linewidth=1, color="black") + scale_linetype_manual(values=c("AR"="solid","MCMC"="dashed","SMC"="dotted")) + labs(title="Posterior density plot: k", x="k", y="Posterior density") + theme_minimal() + theme(legend.title=element_blank())
library(ggpubr)
ggarrange(dentheta, deng, denk, labels = c("A", "B", "C"), ncol = 3, nrow = 1, legend = "bottom", common.legend = TRUE)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
\end{align*}
where $p_{a,M}(\boldsymbol{\eta}(\boldsymbol{y}))$ uses the estimates, which in turn depend on the number of draws $M$ to calculate $\widehat{\boldsymbol{\mu}}_{\boldsymbol{\theta}}$ and $\widehat{\boldsymbol{\Sigma}}_{\boldsymbol{\theta}}$. Note that although we can have unbiased estimators for the mean and covariance, in general, $N(\boldsymbol{\eta}(\boldsymbol{y})\mid \widehat{\boldsymbol{\mu}}_{\boldsymbol{\theta}},\widehat{\boldsymbol{\Sigma}}_{\boldsymbol{\theta}})$ is not an unbiased estimator of $N(\boldsymbol{\eta}(\boldsymbol{y})\mid {\boldsymbol{\mu}}_{\boldsymbol{\theta}},{\boldsymbol{\Sigma}}_{\boldsymbol{\theta}})$. \cite{an2022bsl} show an unbiased estimator for BSL. 
...
RGKnew <- function(par) {
	z <- NULL
	theta <- par[1]; a <- par[2]; b <- par[3]; g <- par[4]
	k <- par[5]; e <- rnorm(n + 1)
	for(t in 2:(n + 1)){
		zt <- e[t] + theta * e[t-1]
		z <- c(z, zt)
...
for (i in 1:12){
	...
	lines(eval, densnorm, col = "black", lty = 2)
}
...
for (s in 2:S){
	...
	if(sum(RestCheck) != 0){
		...
		if(dnormc == -Inf){
			parPost[s,] <- parPost[s-1,]; accept[s] <- 0\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
dentheta <- ggplot(df1, aes(x=Value, linetype=Distribution)) + geom_density(linewidth=1, color="black") + geom_vline(xintercept=theta1, linetype="dashed", color="black", linewidth=1) + scale_linetype_manual(values=c("BSL"="solid","BSLscratch"="dashed")) + labs(title=TeX("Posterior density plot: $\\theta$"), x=TeX("$\\theta$"), y="Posterior density") + theme_minimal() + theme(legend.title=element_blank())
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
deng <- ggplot(df2, aes(x=Value, linetype=Distribution)) + geom_density(linewidth=1, color="black") + geom_vline(xintercept=g, linetype="dashed", color="black", linewidth=1) + scale_linetype_manual(values=c("BSL"="solid","BSLscratch"="dashed")) + labs(title="Posterior density plot: g", x="g", y="Posterior density") + theme_minimal() + theme(legend.title=element_blank())
df3 <- data.frame(Value = c(PostChain[,5], PostChainOwn[,5]), Distribution = factor(c(rep("BSL", Sp), rep("BSLscratch", Sp))))
denk <- ggplot(df3, aes(x=Value, linetype=Distribution)) + geom_density(linewidth=1, color="black") + geom_vline(xintercept=k, linetype="dashed", color="black", linewidth=1) + scale_linetype_manual(values=c("BSL"="solid","BSLscratch"="dashed")) + labs(title="Posterior density plot: k", x="k", y="Posterior density") + theme_minimal() + theme(legend.title=element_blank())
library(ggpubr)
ggarrange(dentheta, deng, denk, labels = c("A", "B", "C"), ncol = 3, nrow = 1, legend = "bottom", common.legend = TRUE)
\end{lstlisting}
\end{VF}
...
The distribution of $\boldsymbol{y}$ is $p(\boldsymbol{y}\mid \boldsymbol{z},\boldsymbol{\theta}_2)$ such that $y_i$ are conditionally independent given $z_i$ and $\boldsymbol{\theta}_2$, $\boldsymbol{\theta}=[\boldsymbol{\theta}_1^{\top} \ \boldsymbol{\theta}_2^{\top}]^{\top}$, $i=1,2,\dots,N$. Thus, the posterior distribution is
...
\end{align*}
These integrals are solve numerically using a smart grid around the mode of $\pi_a(\boldsymbol{\theta}\mid\boldsymbol{y})$. In particular, 
\begin{align*}
...
# Check Rtools is properly installed
# Install INLA
...
inla.la <- inla(formula, data = my.data, family = "poisson", control.inla = list(strategy = "laplace", int.strategy = "grid", dz=0.1, diff.logdens=20), control.compute=list(return.marginals.predictor=TRUE))
summary(inla.sla)
marg_sla <- inla.sla$marginals.fixed$x
marg_ga  <- inla.ga$marginals.fixed$x
marg_la  <- inla.la$marginals.fixed$x
plot(marg_sla, type = "l", col = "black", lwd = 2, xlab = expression(beta[x]), ylab = "Density", main = "Posterior of slope under different INLA strategies")
lines(marg_ga, col = "grey30", lwd = 2, lty = 2)
lines(marg_la, col = "grey50", lwd = 2, lty = 3)
abline(v = 1, col = "grey90", lty = 4, lwd = 2)
legend("topright", legend = c("Simplified Laplace", "Gaussian", "Full Laplace", "True = 1"), col = c("black", "grey30", "grey50", "grey90"), lwd = 2, lty = c(1, 2, 3, 4), bty = "n")
# Summary beta sla
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
# Base plot
plot(marg.var.sla, type = "l", col = "black", lwd = 2, xlab = expression(sigma^2), ylab = "Density", main = "Posterior of Random Effect Variance")
lines(marg.var.ga, col = "grey30", lwd = 2, lty = 2)
lines(marg.var.la, col = "grey50", lwd = 2, lty = 3)
abline(v = 0.25, col = "grey90", lty = 4, lwd = 2)
legend("topright", legend = c("Simplified Laplace", "Gaussian", "Full Laplace", "True Variance (0.25)"), col = c("black", "grey30", "grey50", "grey90"), lwd = 2, lty = c(1, 2, 3, 4), bty = "n")
...
plot(st_geometry(antioquia), border = "black")
plot(nb_object, coords, add = TRUE, col = "grey")
attach(antioquia)
...
semmodels <- mclapply(rrho1, function(rho) {
	sem.inla(fform, d = as.data.frame(antioquia), W = bmsp, rho = rho, family = "gaussian", impacts = FALSE, control.family = list(hyper = zero.variance), control.predictor = list(compute = TRUE), control.compute = list(dic = TRUE, cpo = TRUE), control.inla = list(print.joint.hyper = TRUE))
})
bmasem <- INLABMA(semmodels, rrho1, 0, impacts = FALSE)
#Display results
plot(bmasem$rho$marginal, type="l", col = "black", lwd = 2, xlab = expression(lambda), ylab = "Density", main = "Spatial error model: Posterior spatial coefficient")
bmasem[["rho"]][["quantiles"]]
marg_sla <- bmasem[["marginals.fixed"]][["L_P_OLD"]]
plot(marg_sla, type = "l", col = "black", lwd = 2, xlab = expression(beta[x]), ylab = "Density", main = "Spatial error model: Posterior price elasticity")
...
\begin{figure}[!h]
	\includegraphics[width=200pt]{Chapters/chapter15/figures/AntMapCont.png}
...
\end{align*}
where $(\boldsymbol{z}_i, \boldsymbol{y}_i)$ is a random draw from the sample and its corresponding latent variable. Finally, the step size schedule to update the global parameters is given by
\begin{align*}
