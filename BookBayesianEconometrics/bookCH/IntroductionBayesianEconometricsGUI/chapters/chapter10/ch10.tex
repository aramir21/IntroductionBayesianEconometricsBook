...
The simulation setting implies $2^{27}$ models, which implies approximately 135 million models in the model space. We run our MC3 algorithm using the BIC approximation with 50000 iterations. This takes by far more time that the BIC approximation from the \textit{BMA} package, but it seems to do a good job finding the relevant regressors, and their BMA posterior means and standard deviations. The highest PMP is ?? and is associated with the model ???, the PIPs of the relevant regressors in the population statistical models are equal 1, and the BMA means are ???. The t-ratio are higher than 2 (in absolute value) by far for $x_{i1}$ and $x_{i2}$. The other variables have BMA means near 0, and the t-ratios are also near 0. We ask in Exercise 10 to use this approach to perform a BMA algorithm in the logit regression using the simulation setting of logit models of this section.    
