...
We get the same value, up to two decimals, for the log marginal likelihood of the restricted model using the Chib's method and the analytical expression (-1089.82). The log marginal likelihood of the unrestricted model is -1088.21 using the Chib's method, whereas this is -1087.94 using the analytical expression. Thus, $2\times\log(BF_{21})=3.23$, that is, positive evidence against model $\mathcal{M}_1$ (see Table \ref{tab:guide}).
