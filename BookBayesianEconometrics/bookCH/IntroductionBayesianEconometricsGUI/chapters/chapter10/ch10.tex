...
Generalized linear models (GLM) were introduced by \cite{nelder1972generalized}, and extend the concept of linear regressions to a more general setting. These models are characterized by: i) a dependent variable $\bm{y}$ whose probability distribution function belongs to the exponential family, ii) a linear predictor $\bm{x}^{\top}\bm{\beta}$, and iii) a link function such that $\mathbb{E}[Y|\bm{x}]=g^{-1}(\bm{x}^{\top}\bm{\beta})$, which implies that $g(\mathbb{E}[Y|\bm{x}])=\bm{x}^{\top}\bm{\beta}$.
