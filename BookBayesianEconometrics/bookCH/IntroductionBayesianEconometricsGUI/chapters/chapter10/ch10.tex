...
%	\begin{threeparttable}
		\resizebox{1\textwidth}{!}{\begin{minipage}{\textwidth}
				...
				\begin{tablenotes}[para,flushleft]
					\footnotesize \textit{Notes}: ``1" indicates inclusion of the regressor, and ``0" indicates no inclusion. The space of models is composed of 8 models. The model always includes intercept.\\
				\end{tablenotes}
		\end{minipage}}
%	\end{threeparttable}
\end{table}
...
Following the same steps as in Section \ref{sec43}, the posterior conditional distribution of $\bm{\beta}_m$ has covariance matrix $\sigma^2\bm{B}_{mn}$, where $\bm{B}_{mn}=((1+g_m)\bm{X}_m^{\top}\bm{X}_m)^{-1}$ (Exercise 1), which means that $g_m=0$ implies a non-informative prior, whereas $g_m=1$ implies that prior and data information have the same weights. We follow \cite{fernandez2001benchmark}, who recommend
\begin{align*}
	g_m & =
	\begin{Bmatrix}
		1/K^2, & N \leq K^2\\
...
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
We can see from the results that the BIC approximation with the Occam's window, and the MC3 algorithm do a good job of finding the relevant regressors, and their posterior BMA means are very close to the population values. We also see that the BMA results are very similar in the two approaches.
...
\end{align*}
These conditional Bayes factors assume $\pi(M^{1st},M^{2nd})\propto 1$. See \cite{Lenkoski2013} for more details of the instrumental variable BMA algorithm.\footnote{\cite{Koop12} and \cite{Lenkoski2014} propose other frameworks for BMA taking into account endogeneity.}
...
Bayesian model averaging has also been extended to state-space models. The point of departure is the univariate random walk state-space model (see Equations \ref{eq1Obs} and \ref{eq1St} in Chapter \ref{chap8}) conditional on model $\mathcal{M}_m$, $m=1,2\dots,M$. 
\begin{align}
...
\begin{algorithm}[h!]
	...
	\begin{algorithmic}[1]  		 			
		...
		\State Download posterior results using the \textit{Download results for Dynamic Model Average}. There are four files: the dynamic Bayesian model averaging prediction, the dynamic Bayesian model averaging filtering recursions for each state, and their standard deviation, and the PMP of each model. 
	\end{algorithmic} 
\end{algorithm}
...
plot(dma.test[["pmp"]][-c(1:T0),8], type = "l", main = "Posterior model probability", xlab = "Time", ylab = "PMP")
lines(dma.test[["pmp"]][-c(1:T0),6], lty = 2)
legend(x = 0, y = 0.5, legend = c("Model: All regressors", "Model: Regressors 2 and 4"), lty=1:2, cex=0.8)
require(latex2exp)
plot(dma.test[["thetahat.ma"]][-c(1:T0),1], type = "l", main = "Bayesian model average filtering recursion", xlab = "Time", ylab = TeX("$\\beta_{1}$"))
abline(h = B1, lty = 2)
legend(x = 0, y = 0.4, legend = c("State filtering", "State population"), lty=1:2, cex=0.8)
plot(dma.test[["thetahat.ma"]][-c(1:T0),2], type = "l", main = "Bayesian model average filtering recursion", xlab = "Time", ylab = TeX("$\\beta_{2t}$"), ylim = c(0.5,2))
lines(B2t[-c(1:T0)], lty = 2)
legend(x = 0, y = 2, legend = c("State filtering", "State population"), lty=1:2, cex=0.8)
plot(dma.test[["thetahat.ma"]][-c(1:T0),3], type = "l", main = "Bayesian model average filtering recursion", xlab = "Time", ylab = TeX("$\\beta_{3t}$"))
lines(B3t[-c(1:T0)], lty = 2)
legend(x = 0, y = -0.4, legend = c("State filtering", "State population"), lty=1:2, cex=0.8)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
Figures \ref{figPMPdbma1}, \ref{figPMPdbma2}, \ref{figPMPdbma3}, and \ref{figPMPdbma4} show a comparison between the Bayesian model averaging filtering recursions of the states (solid lines) and their population values (dashed lines). We observe that the filtering recursions follow the general pattern of the population values. However, the values are not perfectly aligned. This discrepancy arises because the posterior model probabilities (PMPs) of the models that match the data-generating process are not equal to 1, which in turn affects the performance of the filtering recursions.  
...
Equation \ref{eq:SD} is called the Savage-Dickey density ratio. A nice feature is that just requires estimation of model $\mathcal{M}_2$, and evaluation of the prior and posterior densities. This means that no evaluation of the marginal likelihood is required \cite[Chap.~4]{koop2003bayesian}.\\
...
Observe that we can calculate the marginal likelihoods of the models in Chapters \ref{chap6}, \ref{chap7}, \ref{chap8} and \ref{chap9} using the Chib's methods and the Gelfand-Dey method.\\
