...
We leave other useful Bayesian non-parametric approaches like Bayesian additive random trees (BART) and Gaussian process (GP) for Chapter \ref{chap13}. 
...
where $\sigma_{hn}^2=\left(\frac{1}{\sigma_{h 0}^{2}}+\frac{N_h}{\sigma_{h}^2}\right)^{-1}$ and $\mu_{hn}=\sigma_{hn}^2\left(\frac{\mu_{h0}}{\sigma_{h0}^2}+\frac{\sum_{\left\{i:\psi_{ih}=1\right\}} (y_i-\boldsymbol{x}_i\boldsymbol{\beta})}{\sigma_h^2}\right)$.
...
### Simulation exercise: Label switching issue ###
rm(list = ls()); set.seed(10101); library(ggplot2)
# Simulate data from a 2-component mixture model
n <- 500; z <- rbinom(n, 1, 0.75)  # Latent class indicator
...
# Plot
require(latex2exp)
ggplot(dfMU)+geom_density(aes(x=mu1,linetype="mu1"),linewidth=1,color="black")+geom_density(aes(x=mu2,linetype="mu2"),linewidth=1,color="black")+scale_linetype_manual(values=c("mu1"="solid","mu2"="dashed"),name="Variable")+labs(title="DensityPlot",x=TeX("$\\mu$"),y="Density")+theme_minimal()
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
# Plot
require(latex2exp)
ggplot(dfMU)+geom_density(aes(x=mu1,linetype="mu1"),color="black",linewidth=1)+geom_density(aes(x=mu2,linetype="mu2"),color="black",linewidth=1)+scale_linetype_manual(values=c("mu1"="solid","mu2"="dashed"),name="Variable")+labs(title="DensityPlot",x=TeX("$\\mu$"),y="Density")+theme_minimal()
# Custom colors
...
labs(title = "Density Plot", x = "y", y = "Density") + theme_minimal()
# Define priors
priors <- c(
...
geom_density(fill = "blue", alpha = 0.3) +  # Density plot with fill color
labs(title = "Density Plot", x = "Residuals", y = "Density") + theme_minimal()
# Hyperparameters
...
Note that the hierarchical representation induces specific unit parameters, leading to a probabilistic clustering model \cite{Antoniak1974}, similar to a finite Gaussian mixture. However, the DPM is not consistent in estimating the number of clusters, it tends to overestimate the number of clusters; although there is posterior asymptotic concentration in the other model components \cite{Miller2014}.
...
To sum up, a Dirichlet process mixture is a flexible way to model non-parametrically a distribution using an infinite weighted sum of discrete distributions, where each individual weight increases with the number of observations that belong to it.\\
...
We see from the results that the posterior plots illustrate the label-switching problem. Eventually, the posterior chains of clusters reach different posterior modes, causing a high variability of the posterior draws. In Exercise 5, we ask to fix this issue using \textit{random permutation of latent classes}.
...
y <- ifelse(z == 1, rnorm(N, 2 + 1.5*x, 1), rnorm(N, -1 + 0.5*x, 0.8))
X <- cbind(1, x); k <- 2
data <- data.frame(y, x); Reg <- lm(y ~ x); SumReg <- summary(Reg)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
SIGMA <- rep(SumReg$sigma, 3); Alpha <- rgamma(1, a, b)
pb <- txtProgressBar(min = 0, max = tot, style = 3)
for(s in 1:tot){
	for(i in 1:N){
...
dens1 <- ggplot(DataElast, aes(x = Marijuana)) + geom_density(fill = "grey", alpha = 0.3) + labs(x = "Marijuana", y = "Density") + theme_minimal()
dens2 <- ggplot(DataElast, aes(x = Cocaine)) + geom_density(fill = "grey", alpha = 0.3) +  labs(x = "Cocaine", y = "Density") + theme_minimal()
dens3 <- ggplot(DataElast, aes(x = Crack)) + geom_density(fill = "grey", alpha = 0.3) + labs(x = "Crack", y = "Density") + theme_minimal()
library(ggpubr)
...
par(mfrow=c(1,2));lt<-c(1,2,3,4,5,6);lt<-rep(lt,length.out=ncol(basis_matrix))
matplot(xA,basis_matrix,type="l",lty=lt,col="black",ylab="B-splineBasis",xlab="x",main="Ownfunction")
matplot(xA,bs_matrix_matrix,type="l",lty=lt,col="black",ylab="B-splineBasis",xlab="x",main="bsfunction")
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
The idea in splines is to calculate the linear combination of the $H$ B-splines, as those shown in Figure \ref{figBsplines}, that best fit the dependent variable.
...
\begin{tcolorbox}[enhanced,width=4.67in,center upper,
	...
	\begin{VF}
		\begin{lstlisting}[language=R]
ggplot(data, aes(x = x, y = y)) + geom_line(data = subset(data, Type == "True Signal"), aes(color = "True Signal"), linewidth = 1) + geom_point(data = subset(data, Type == "Observed Data"), aes(color = "Observed Data"), shape = 16) + geom_line(data = subset(data, Type == "Spline 1"), aes(color = "Splines"), linewidth = 1, linetype = "dashed") + geom_line(data = subset(data, Type == "Spline 2"), aes(color = "Splines"), linewidth = 1, linetype = "dashed") + geom_line(data = subset(data, Type == "Spline 3"), aes(color = "Splines"), linewidth = 1, linetype = "dashed") + geom_line(data = subset(data, Type == "Spline 4"), aes(color = "Splines"), linewidth = 1, linetype = "dashed") + scale_color_manual(values = c("True Signal" = "black", "Observed Data" = "black", "Splines" = "black")) + labs(y = "y", color = "Legend") + theme_minimal() + theme(legend.position = "top")
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
# Create ggplot
ggplot(data,aes(x=x,y=y))+ geom_line(data=subset(data,Type=="True Signal"),aes(linetype="TrueSignal"),color="black",linewidth=1)+ geom_point(data=subset(data,Type=="Observed Data"),aes(shape="ObservedData"),color="black",size=2)+ geom_line(data=subset(data,grepl("Fit",Type)),aes(linetype=Type),color="black",linewidth=1)+ scale_linetype_manual(values=c("TrueSignal"="solid","Fit 1"="dashed","Fit 2"="dotted","Fit 3"="dotdash","Fit 4"="longdash"),name="Legend")+ scale_shape_manual(values=c("ObservedData"=16),name="Legend")+ labs(y="y")+ theme_minimal()+ theme(legend.position="top")
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
k <- dim(X)[2]; N <- dim(X)[1]
# Hyperparameters
d0 <- 0.001; a0 <- 0.001; b0 <- rep(0, k)
c0 <- 1000; B0 <- c0*diag(k); B0i <- solve(B0)
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
...
ggplot() + geom_line(data = plot_data, aes(x, fit), color = "black", linewidth = 1) + geom_ribbon(data = plot_data, aes(x, ymin = liminf, ymax = limsup), fill = "grey10", alpha = 0.2) + labs(title = "B-Spline Regression with 95% Confidence Interval", x = "Age", y = "Log Marijuana") + theme_minimal()
\end{lstlisting}
	\end{VF}
\end{tcolorbox}
