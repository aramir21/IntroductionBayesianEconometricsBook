...
\begin{align}
	...
	&=\frac{N\times n! \Gamma\left(\sum_{l=1}^n\nonumber \alpha_{0l}\right)}{\Gamma\left(\sum_{l=1}^n \alpha_{0l}+N\times n\right)}\prod_{l=1}^m \frac{\Gamma\left( \alpha_{nl}\right)}{\Gamma\left(\alpha_{0l}\right)\prod_{i=1}^N y_{il}!},\nonumber
\end{align}
...
Observe that as we have incorporated the uncertainty of the variance, the posterior for $\beta$ changes from a normal to a Students' t distribution, which has heavier tails, indicating more uncertainty. 
...
The posterior predictive is a multivariate Student's t, ${\bf{Y}}_0|{\bf{y}}\sim t\left({\bf{X}}_0\beta_n,\frac{\delta_n({\bf{I}}_{N_0}+{\bf{X}}_0{\bf{B}}_n{\bf{X}}_0^{\top})}{\alpha_n},\alpha_n\right)$. 
...
\end{align*}
That is $\pi({\bf{B}},{\bf \Sigma}| {\bf{Y}}, {\bf{X}})=\pi ({\bf{B}}| {\bf \Sigma},{\bf{Y}},{\bf{X}})\pi({\bf \Sigma}| {\bf{Y}},{\bf{X}})$ where $\pi({\bf{B}}| {\bf \Sigma},{\bf{Y}}, {\bf{X}}) \sim N_{K\times M}({\bf{B}}_n,{\bf{V}}_n,{\bf \Sigma})$ and $\pi({\bf \Sigma}| {\bf{Y}},{\bf{X}}) \sim IW({\bf{\Psi}}_n,{\alpha}_n)$
where $\alpha_n= N+\alpha_{0}$.

The marginal posterior for ${\bf{B}}$ is given by

\begin{align*}
	\pi({\bf{B}}|{\bf{Y}},{\bf{X}})&\propto \int_{\bf{\mathcal{S}}} \left|{\bf \Sigma} \right|^{-(\alpha_n+K+M+1)/2}\exp\left\lbrace -\frac{1}{2} tr\left\{\left[({\bf{B}}-{\bf{B}}_n)^{\top}{\bf{V}}_n^{-1}({\bf{B}}-{\bf{B}}_n)+{\bf{\Psi}}_n \right]  {\bf \Sigma}^{-1}\right\}\right\rbrace d{\bf{\Sigma}} \\
...
\begin{align*}
	...
	&=|{\bf{\Psi}}_n|^{-(\alpha_n+K)/2}(2\pi)^{-M(N+K)/2}\left|{\bf V}_0\right|^{-M/2}\frac{|\Psi_0|^{\alpha_0/2}}{2^{\alpha_0M/2}\Gamma_M(\alpha_0/2)}2^{M(\alpha_n+K)/2}\Gamma_M((\alpha_n+K)/2)\\
