...
Bhn <- array(c(1,0,0,1,1,0,0,1), c(K, K, H))
an <- c(500, 500)
VarParPsi <- function(dhn, ahn, bhn, Bhn, an, X, y){
  n <- length(y)
  logrhoih <- matrix(NA, n, H)
  for(i in 1:n){
    for(h in 1:H){
...
J <- 200
ids <- sample(1:n, J)
Xbatch <- X[ids,]; ybatch <- y[ids] 
Rih <- VarParPsi(dhn = dhn, ahn = ahn, bhn = bhn, Bhn = Bhn, an = an, X = Xbatch, y = ybatch)
VarParLambda <- function(rih){
  an <- a0 + colSums(rih)
  return(an)
}
VarParLambda(rih = Rih)
VarParBetah <- function(ahn, dhn, rh, X, y){
  n <- length(y)
  rXX <- matrix(0, K, K)
  rXy <- matrix(0, K, 1)
  for(i in 1:n){
...
h <- 2
VarParBetah(ahn = ahn[h], dhn = dhn[h], rh = Rih[,h], X = Xbatch, y = ybatch)
VarParSigma2h <- function(bhn, Bhn, rh, X, y){
  n <- length(y)
  ahn <- a0h + sum(rh)
  dhnterm <- 0
  for(i in 1:n){
...
h <- 1
VarParSigma2h(bhn = bhn[,h], Bhn = Bhn[,,h], rh = Rih[,h], X = Xbatch, y = ybatch)
# Auxiliar function to calculate the log multivariate beta function
# Define Gamma and Beta functions
multivariate_beta <- function(alpha) {
  prod_gammas <- sum(sapply(alpha, lgamma))
...
ELBO <- function(rih, dhn, ahn, bhn, Bhn, an, X, y){
  n <- length(y)
  LogLikh <- 0; LogPrioPsih <- 0
  LogPriorBeta <- 0; LogPriorSig2 <- 0
  for(h in 1:H){
...
ELBO(rih = Rih, dhn = dhn, ahn = ahn, bhn = bhn, Bhn = Bhn, an = an, X = Xbatch, y = ybatch)
# Initial values
# Use rough values from data (not cheating, just guiding)
fit_lm1 <- lm(y ~ x)
bhn[,1] <- coef(fit_lm1)
...
S <- 1000
ELBOs <- rep(NA, S)
ELBOs[1] <- ELBO(rih = Rih, dhn = dhn, ahn = ahn, bhn = bhn, Bhn = Bhn, an = an, X = Xbatch, y = ybatch)
epsilon <- 1e-5; kp <- 0.75; et <- 1 
J <- 100; prop <- n/J
pb <- winProgressBar(title = "progress bar", min = 0, max = S, width = 300)
tick <- Sys.time()
for(s in 2:S){
  ...
  ids <- sample(1:n, J)
  Xbatch <- X[ids,]; ybatch <- y[ids] 
  Rihs <- VarParPsi(dhn = dhn, ahn = ahn, bhn = bhn, Bhn = Bhn, an = an, X = Xbatch, y = ybatch)
  Rih <- matrix(NA, n, H)
  for(h in 1:H){
    Rih[,h] <- rep(Rihs[,h], prop)
  }
  ans <- VarParLambda(rih = Rih)
  an <- (1 - et)*an + et*ans
  bhns <- matrix(NA, K, H)
  Bhns <- array(NA, c(K, K, H))
  for(h in 1:H){
    EnsVarParBetah <- VarParBetah(ahn = ahn[h], dhn = dhn[h], rh = Rih[,h], X = X, y = y)
    bhns[,h] <- EnsVarParBetah[["bhn"]]
    Bhns[,,h] <- EnsVarParBetah[["Bhn"]] 
    bhn[,h] <- (1 - et)*bhn[,h] + et*bhns[,h]
    Bhn[,,h] <- (1 - et)*Bhn[,,h] + et*Bhns[,,h]
  }
  ahns <- rep(NA, H)
  dhns <- rep(NA, H)
  for(h in 1:H){
    EnsVarParSig2h <- VarParSigma2h(bhn = bhn[,h], Bhn = Bhn[,,h], rh = Rih[,h], X = X, y = y)
    ahns[h] <- EnsVarParSig2h[["ahn"]]
    dhns[h] <- EnsVarParSig2h[["dhn"]]
    ahn[h] <- (1 - et)*ahn[h] + et*ahns[h]
    dhn[h] <- (1 - et)*dhn[h] + et*dhns[h]
  }
  ELBOs[s] <- ELBO(rih = Rih, dhn = dhn, ahn = ahn, bhn = bhn, Bhn = Bhn, an = an, X = Xbatch, y = ybatch)
  # # Check if lower bound decreases
  # if (ELBOs[s] < ELBOs[s - 1]) { message("Lower bound decreases!\n")}
  # # Check for convergence
  # if (ELBOs[s] - ELBOs[s - 1] < epsilon) { break }
