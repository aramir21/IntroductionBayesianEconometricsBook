...
We leave other useful Bayesian non-parametric approaches like Bayesian additive random trees (BART) and Gaussian process (GP) for Chapter \@ref(Chap12).
...
Another common approach to performing non-parametric or semi-parametric inference is using splines (see for instance @chib2010additive). These are piecewise polynomial functions that allow for approximating complex relationships in data.
...
The idea in splines is to calculate the linear combination of the $H$ B-splines, as those shown in the figure, that best fit the dependent variable. 
