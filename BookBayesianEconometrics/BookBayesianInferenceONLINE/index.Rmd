...
knitr::include_graphics('figures/CoverOnline.jpeg', dpi = NA)
```

Since the late 90s, Bayesian inference has gained significant popularity among researchers due to the computational revolution and the availability of algorithms to solve complex integrals. However, many researchers, students, and practitioners still lack a deep understanding and practical application of this inferential approach. The primary reason for this is the requirement for strong programming skills.^[On the cover, from left to right, are Bayes, Laplace, and Price. The portrait of Bayes is the commonly circulated image of Thomas Bayes; however, there is no reliable historical evidence confirming its authenticity.]
...
4. First, install **Docker Desktop**. Then, open **Docker Desktop**, and, use a **Docker** image by typing in the **Command Prompt**:

- `docker pull aramir21/besmartergui:latest`
- `docker run --rm -p 3838:3838 aramir21/besmartergui`

Then, users can access our GUI at **http://localhost:3838/**. See Chapter \@ref(Chap5) for details. In addition, we provide video tutorials on our YouTube channel (**https://www.youtube.com/@besmarterteam**) that demonstrate how to use the GUI.
...
To my wife, Estephania, for her unwavering love and support. To my parents, Orlando and Nancy, who have always been there for me with their unconditional support. They have taught me that the primary aspect of human spiritual evolution is humility, a lesson I am still learning every day.
