...
Then, we can *pointidentified* the LATE under exclusion restriction,
...
@chib2025potential proposes a Bayesian inferential framework for DiD designs with staggered treatments. Their approach can be implemented using the *bdid* package.^[Available at *https://siddharthachib.org/rpackages/bdid/*.] 
...
A difference in this example is that, unlike most of the Frequentist literature, the entire dataset on the forcing variable is used. @chib2023nonparametric propose a non-parametric Bayesian framework that emphasizes the data near the cutoff using a *soft-window* approach.^[The *bayesrdd* package, available at *https://siddharthachib.org/rpackages/rdd/*, can be used to perform nonparametric RD following @chib2023nonparametric.] Other Bayesian approaches to inference in RD include @branson2019nonparametric, who use Gaussian process regression to model the conditional expectation function, and @rischard2020bayesian, who develop a non-parametric Bayesian framework for spatial RD.
...
If $d>p$ the model is *overidentified*; if $d=p$ it is *exactly identified* (and if $d<p$ it is *under-identified*).  
...
to identify the demand and supply functions. Note that the demand equation is *exactly identified*, whereas the supply equation is *overidentified*.
