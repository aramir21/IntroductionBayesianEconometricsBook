...
In particular, we set \( \pi(\mu, \sigma) = \pi(\mu) \pi(\sigma) \propto \frac{1}{\sigma} \). This is a standard *non-informative improper* prior (Jeffreys prior, see Chapter \@ref(Chap3)). That is, this prior is perfectly compatible with the sample information. Additionally, we assume independent priors for \( \mu \) and \( \sigma \).
...
The first term in the last expression is the kernel of a normal density, \( \mu|\sigma,\mathbf{y} \sim N(\bar{y}, \sigma^2 / N) \). The second term is the kernel of an inverted gamma density [@zellner1996introduction], \( \sigma|\mathbf{y} \sim IG(\alpha_n, \hat{\sigma}^2) \). Therefore, 
