...
There are three ways to install our GUI. The simplest method—requiring only the installation of **R** and, optionally, an **R** code editor—is to type `shiny::runGitHub("besmarter/BSTApp", launch.browser=T)` in the **R** package console or any **R** code editor. We strongly recommend typing this command manually rather than copying and pasting it, as quotation marks can sometimes cause issues.
